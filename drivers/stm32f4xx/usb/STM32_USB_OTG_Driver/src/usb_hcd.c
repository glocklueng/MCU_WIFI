...
URB_STATE HCD_SubmitRequest(struct urb *urb, USB_OTG_CORE_HANDLE *pdev, uint8_t
	hc_num)
{
	URB_STATE ret;
	...
	pdev->host.hc[hc_num].packet_num = (pdev->host.hc[hc_num].xfer_len + 63)/64;

	if (pdev->host.hc[hc_num].ep_type == USB_OTG_EP_CONTROL)
	{
		if (OSIntNesting)
			p_err("HCD_SubmitRequest in_interrupt\n");
	...
	if (pdev->host.hc[hc_num].ep_type == USB_OTG_EP_CONTROL)
	{
		ret = usb_start_wait_urb(pdev, hc_num, 300);
	}
	else
	{
		ret = URB_DONE;
