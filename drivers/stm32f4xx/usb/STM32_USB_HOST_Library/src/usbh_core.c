...
void USBH_Init(USB_OTG_CORE_HANDLE *pdev, USB_OTG_CORE_ID_TypeDef coreID,
    USBH_HOST *phost, USBH_Class_cb_TypeDef *class_cb, USBH_Usr_cb_TypeDef
    *usr_cb)
{
 ...
 */
    ...
    void USBH_Process(USB_OTG_CORE_HANDLE *pdev, USBH_HOST *phost)
    {
        ...
        switch (phost->gState)
        {
            ...
            case HOST_DEV_ATTACHED:
                ...
                if (HCD_ResetPort(pdev) == 0)
                {
                    ...
                    /* Open Control pipes */
                    USBH_Open_Channel(pdev, phost->Control.hc_num_in, phost
                        ->device_prop.address, phost->device_prop.speed,
                        EP_TYPE_CTRL, phost->Control.ep0size);

                    /* Open Control pipes */
                    USBH_Open_Channel(pdev, phost->Control.hc_num_out, phost
                        ->device_prop.address, phost->device_prop.speed,
                        EP_TYPE_CTRL, phost->Control.ep0size);
                }
                break;
        ...
        if (!(HCD_IsDeviceConnected(pdev)) && (pdev->host.port_cb
            ->DisconnHandled == 0))
        {
...
//    phost->EnumState = ENUM_GET_FULL_DEV_DESC;
    ...
    USBH_Modify_Channel(pdev, phost->Control.hc_num_out, 0, 0, 0, phost
        ->Control.ep0size);

    USBH_Modify_Channel(pdev, phost->Control.hc_num_in, 0, 0, 0, phost
        ->Control.ep0size);
    ...
    USBH_Modify_Channel(pdev, phost->Control.hc_num_in, phost
        ->device_prop.address, 0, 0, 0);

    USBH_Modify_Channel(pdev, phost->Control.hc_num_out, phost
        ->device_prop.address, 0, 0, 0);
    ...
    if (USBH_Get_CfgDesc(pdev, phost, phost->device_prop.Cfg_Desc.wTotalLength)
        != USBH_OK)
    {
        return Status;
    }
    ...
    if (phost->device_prop.Dev_Desc.iManufacturer != 0)
    {
        ...
        if (USBH_Get_StringDesc(pdev, phost, phost
            ->device_prop.Dev_Desc.iManufacturer, Local_Buffer, 0xff) !=
            USBH_OK){
    ...
    if (phost->device_prop.Dev_Desc.iProduct != 0)
    {
        ...
        if (USBH_Get_StringDesc(pdev, phost, phost
            ->device_prop.Dev_Desc.iProduct, Local_Buffer, 0xff) == USBH_OK){
    ...
    if (phost->device_prop.Dev_Desc.iSerialNumber != 0)
    {
        ...
        if (USBH_Get_StringDesc(pdev, phost, phost
            ->device_prop.Dev_Desc.iSerialNumber, Local_Buffer, 0xff) ==
            USBH_OK){

        }
    }

    if (USBH_SetCfg(pdev, phost, phost
        ->device_prop.Cfg_Desc.bConfigurationValue) == USBH_OK)
    {
...
USBHST_STATUS(*Complete)(URBCompleteStatus Status1, purbb_t pURB, pregs
    *pt_regs);
...
void wake_up_urb(USB_OTG_CORE_HANDLE *pdev, int num)
{
    ...
    else if (pdev->urb[num] && pdev->urb[num]->complete)
    {
			if (pdev->host.URB_State[num] == URB_DONE)
				...
				pdev->urb[num]->status = pdev->host.URB_State[num];
			pdev->urb[num]->actual_length = pdev->host.hc[num].xfer_count;

			Complete = (USBHST_STATUS (*)(URBCompleteStatus, purbb_t, pregs *))pdev->urb[num]->complete;
			...
			Complete(pdev->host.URB_State[num], pdev->urb[num], 0);
			pdev->urb[num] = 0;
			if(!pdev->host.hc[num].ep_is_in)
...
URB_STATE usb_start_wait_urb(USB_OTG_CORE_HANDLE *pdev, int hc_num, int timeout)
{
    ...
    if (ret == -1)
    {
		...
		p_err("hctsiz.xfersize:%d, hctsiz.pktcnt:%d, hnptxsts.fspc:%d, hnptxsts.qspc:%d",
			hctsiz.b.xfersize,
			hctsiz.b.pktcnt,
			hnptxsts.b.nptxfspcavail,
			hnptxsts.b.nptxqspcavail
			
			);

		hctsiz.d32 = USB_OTG_READ_REG32(&pdev->regs.HC_REGS[3]->HCTSIZ);

 		p_err("hctsiz.xfersize:%d, hctsiz.pktcnt:%d, %d, %d, %08x",
			hctsiz.b.xfersize,
			hctsiz.b.pktcnt,
			pdev->host.hc[3].packet_count,
			pdev->host.hc[3].xfer_len,
			ret);
...
URB_STATE USBH_HandleControl_Setup(USB_OTG_CORE_HANDLE *pdev, USBH_HOST *phost,
    int timeout)
{
    USBH_Status state;
    state = USBH_CtlSendSetup(pdev, phost->Control.setup.d8, phost
        ->Control.hc_num_out);
...
URB_STATE USBH_HandleControl_Data_in(USB_OTG_CORE_HANDLE *pdev, USBH_HOST
    *phost, int timeout)
{
    USBH_Status state;
    state = USBH_CtlReceiveData(pdev, phost->Control.buff, phost
        ->Control.length, phost->Control.hc_num_in);
...
URB_STATE USBH_HandleControl_Data_out(USB_OTG_CORE_HANDLE *pdev, USBH_HOST
    *phost, int timeout)
{
    USBH_Status state;
    ...
    p_dbg("toggle_out:%d,%d\n", pdev->host.hc[phost
        ->Control.hc_num_out].toggle_out, phost->Control.hc_num_out);

    state = USBH_CtlSendData(pdev, phost->Control.buff, phost->Control.length,
...
URB_STATE USBH_HandleControl_Status_in(USB_OTG_CORE_HANDLE *pdev, USBH_HOST
    *phost, int timeout)
{
    USBH_Status state;
...
URB_STATE USBH_HandleControl_Status_out(USB_OTG_CORE_HANDLE *pdev, USBH_HOST
    *phost, int timeout)
{
    USBH_Status state;
    pdev->host.hc[phost->Control.hc_num_out].toggle_out ^= 1;
