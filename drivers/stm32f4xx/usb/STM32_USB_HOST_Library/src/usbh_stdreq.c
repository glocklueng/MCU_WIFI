 ...
 * @{
 */
static void USBH_ParseDevDesc(USBH_DevDesc_TypeDef *, uint8_t *buf, uint16_t
    length);

static void USBH_ParseCfgDesc(USBH_CfgDesc_TypeDef *cfg_desc,
    USBH_InterfaceDesc_TypeDef *itf_desc, USBH_EpDesc_TypeDef *ep_desc, uint8_t
    *buf, uint16_t length);
 ...
 * @retval Status
 */
USBH_Status USBH_Get_DevDesc(USB_OTG_CORE_HANDLE *pdev, USBH_HOST *phost,
    uint8_t length)
{
	...
	if(!pdev->host.Rx_Buffer)
		return USBH_FAIL;
    if ((status = USBH_GetDescriptor(pdev, phost, USB_REQ_RECIPIENT_DEVICE |
        USB_REQ_TYPE_STANDARD, USB_DESC_DEVICE, pdev->host.Rx_Buffer, length))
        == USBH_OK)
    {
        /* Commands successfully sent and Response Received */
  //      p_hex("DevDesc", pdev->host.Rx_Buffer, length);
        USBH_ParseDevDesc(&phost->device_prop.Dev_Desc, pdev->host.Rx_Buffer,
            length);
    }
 ...
 * @retval Status
 */
USBH_Status USBH_Get_CfgDesc(USB_OTG_CORE_HANDLE *pdev, USBH_HOST *phost,
    uint16_t length)

{
	...
	if(!pdev->host.Rx_Buffer)
		return USBH_FAIL;
    if ((status = USBH_GetDescriptor(pdev, phost, USB_REQ_RECIPIENT_DEVICE |
        USB_REQ_TYPE_STANDARD, USB_DESC_CONFIGURATION, pdev->host.Rx_Buffer,
        length)) == USBH_OK)
    {
        /* Commands successfully sent and Response Received  */
        USBH_ParseCfgDesc(&phost->device_prop.Cfg_Desc, phost
            ->device_prop.Itf_Desc, phost->device_prop.Ep_Desc[0], pdev
            ->host.Rx_Buffer, length);
 ...
 * @retval Status
 */
USBH_Status USBH_Get_StringDesc(USB_OTG_CORE_HANDLE *pdev, USBH_HOST *phost,
    uint8_t string_index, uint8_t *buff, uint16_t length)
{
	...
	if(!pdev->host.Rx_Buffer)
		return USBH_FAIL;
    if ((status = USBH_GetDescriptor(pdev, phost, USB_REQ_RECIPIENT_DEVICE |
        USB_REQ_TYPE_STANDARD, USB_DESC_STRING | string_index, pdev
 ...
 * @retval Status
 */
USBH_Status USBH_GetDescriptor(USB_OTG_CORE_HANDLE *pdev, USBH_HOST *phost,
 ...
 * @retval Status
 */
USBH_Status USBH_SetAddress(USB_OTG_CORE_HANDLE *pdev, USBH_HOST *phost,
...
short USBH_GetStatus(USB_OTG_CORE_HANDLE *pdev, USBH_HOST *phost, uint8_t
    ep_num, uint8_t req_type)
{
    short status;
    p_dbg_enter;
    phost->Control.setup.b.bmRequestType = USB_D2H | req_type;
 ...
 * @retval Status
 */
USBH_Status USBH_SetCfg(USB_OTG_CORE_HANDLE *pdev, USBH_HOST *phost, uint16_t
    cfg_idx)
{
...
USBH_Status USBH_SetFeature(USB_OTG_CORE_HANDLE *pdev, USBH_HOST *phost,
 ...
 * @retval Status
 */
USBH_Status USBH_ClrFeature(USB_OTG_CORE_HANDLE *pdev, USBH_HOST *phost,
    uint8_t ep_num, uint8_t hc_num)
{
...
USBH_Status USBH_ClrFeature1(USB_OTG_CORE_HANDLE *pdev, USBH_HOST *phost,
    uint8_t ep_num, uint8_t hc_num)
{
 ...
 * @retval None
 */
static void USBH_ParseDevDesc(USBH_DevDesc_TypeDef *dev_desc, uint8_t *buf,
    uint16_t length)
{
