 ...
 * @retval Status
 */
uint8_t USBH_Open_Channel(USB_OTG_CORE_HANDLE *pdev, uint8_t hc_num, uint8_t
    dev_address, uint8_t speed, uint8_t ep_type, uint16_t mps)
{
    p_dbg("%s hc_num:%d, addr:%d, speed:%d, type:%d, mps:%d\n", __FUNCTION__,
        hc_num, dev_address, speed, ep_type, mps);
 ...
 * @retval Status
 */
uint8_t USBH_Modify_Channel(USB_OTG_CORE_HANDLE *pdev, uint8_t hc_num, uint8_t
    dev_address, uint8_t speed, uint8_t ep_type, uint16_t mps)
{
