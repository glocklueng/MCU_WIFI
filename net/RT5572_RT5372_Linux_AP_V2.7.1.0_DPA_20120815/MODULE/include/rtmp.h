...
extern  const unsigned char   CISCO_OUI[];
extern  const UCHAR	BaSizeArray[4];

extern const UCHAR BROADCAST_ADDR[MAC_ADDR_LEN];
extern const UCHAR ZERO_MAC_ADDR[MAC_ADDR_LEN];
extern const ULONG BIT32[32];
extern char *CipherName[];
extern const UCHAR RxwiMCSToOfdmRate[12];
extern const UCHAR SNAP_802_1H[6];
extern const UCHAR SNAP_BRIDGE_TUNNEL[6];
extern const UCHAR EAPOL[2];
extern const UCHAR IPX[2];
extern const UCHAR TPID[];
extern const UCHAR APPLE_TALK[2];
extern const UCHAR OfdmRateToRxwiMCS[];
extern const UCHAR MapUserPriorityToAccessCategory[8];
extern const UCHAR BIT8[8];
...
extern const unsigned char RateIdToMbps[];
extern const USHORT RateIdTo500Kbps[];
...
extern const UCHAR SsidIe;
extern const UCHAR SupRateIe;
extern const UCHAR ExtRateIe;

#ifdef DOT11_N_SUPPORT
extern const UCHAR HtCapIe;
extern const UCHAR AddHtInfoIe;
extern const UCHAR NewExtChanIe;
extern const UCHAR BssCoexistIe;
extern const UCHAR ExtHtCapIe;
#endif /* DOT11_N_SUPPORT */
extern const UCHAR ExtCapIe;

extern const UCHAR ErpIe;
extern const UCHAR DsIe;
extern const UCHAR TimIe;
extern const UCHAR WpaIe;
extern const UCHAR Wpa2Ie;
extern const UCHAR IbssIe;
extern const UCHAR WapiIe;

extern const UCHAR WPA_OUI[];
extern const UCHAR RSN_OUI[];
extern const UCHAR WAPI_OUI[];
extern const UCHAR WME_INFO_ELEM[];
extern const UCHAR WME_PARM_ELEM[];
extern const UCHAR RALINK_OUI[];
extern const UCHAR PowerConstraintIE[];
...
{																\
	...
	{															\
		...
		if (NdisEqualMemory((PUCHAR)IPX, _pBufVA + 12, 2) || 			\
			NdisEqualMemory((PUCHAR)APPLE_TALK, _pBufVA + 12, 2))		\
		{														\
...
{																\
	...
	{															\
		_pExtraLlcSnapEncap = (PUCHAR)SNAP_802_1H;						\
		if (NdisEqualMemory((PUCHAR)IPX, _pBufVA, 2) || 				\
			NdisEqualMemory((PUCHAR)APPLE_TALK, _pBufVA, 2))			\
		{														\
			_pExtraLlcSnapEncap = (PUCHAR)SNAP_BRIDGE_TUNNEL;			\
		}														\
	}															\
...
struct _RTMP_ADAPTER {
	...
	wait_event_t tasklet_thread_event;
#if USE_SIMPLE_TRANSMIT	
	wait_event_t data_bulk_out_wait;
	mutex_t data_bulk_out_lock;
#endif
	mutex_t hard_transmit_lock;
	mutex_t mlme_transmit_lock;
	mutex_t deque_transmit_lock;
	
	//char 	connect_by_bssentry_key[33];
	//BOOL	connect_by_bssentry_pending;
	//BOOL	connect_by_bssentry_ready;
	//uint8_t connect_by_bssentry_retry_cnt;
...
/*	struct usb_config_descriptor		*config; */
	VOID *config;
#ifdef MT7601
	UINT BulkInEpAddr[2];	/* bulk-in endpoint address */
#else
	UINT BulkInEpAddr;
#endif
...
/*	RTMP_OS_TASK			cmdQTask; */
	...
	//PVOID UsbVendorReqBuf; //del  by lck
...
#if USE_14_CHANNELS
	uint32_t txpower_critcal_flag;
#endif
	CHANNEL_TX_POWER ChannelList[MAX_NUM_OF_CHANNELS];	/* list all supported channels for site survey */
#if USE_14_CHANNELS
	uint32_t channellist_critcal_flag;
#endif
...
#ifdef RTMP_MAC_USB
	TX_NULL_CONTEXT NullContext;
	TX_NULL_CONTEXT PsPollContext;
#endif /* RTMP_MAC_USB */
...
/*=======Common=========== */
	...
	*/
	...
	UCHAR default_addr[MAC_ADDR_LEN]; //add by lck,生成的随机mac地址
	...
	CIPHER_KEY SharedKey[HW_BEACON_MAX_NUM + MAX_P2P_NUM][SHARE_KEY_NUM];	/* STA always use SharedKey[BSS0][0..3] */

	uint32_t SharedKey_critcal_flag;	
	...
	if(pFrame->FC.Type == BTYPE_MGMT)
	{
		switch(pFrame->FC.SubType)
		{
			...
			case SUBTYPE_REASSOC_REQ:
				...
				pMacHdr += (sizeof(HEADER_802_11)  - 1/*wsum*/);
			...
			case SUBTYPE_PROBE_RSP:
				...
				pMacHdr += ((sizeof(HEADER_802_11)  - 1/*wsum*/) + TIMESTAMP_LEN);
				*(USHORT *)pMacHdr = SWAP16(*(USHORT *)pMacHdr);
...
MLME_QUEUE_ELEM *alloc_mlme_elem(int msg_size);
void cpy_mlme_elem(MLME_QUEUE_ELEM *to, MLME_QUEUE_ELEM *from);
void free_mlme_elem(MLME_QUEUE_ELEM *Elem);
...
{																						\
	...
	{                                                                           											\
		...
		_pSA = (PUCHAR)_pRxBlk->pHeader + (sizeof(HEADER_802_11)  - 1/*wsum*/);                					\
	}                                                                           											\
