...
typedef sys_sem_t	OS_SEM;

typedef int (*HARD_START_XMIT_FUNC)(struct sk_buff *skb, struct net_device *net_dev);

typedef int (*_wireless_event_callback)(int type);
...
#if 1
extern uint32_t  OS_SEM_LOCK_IRQ_FLAG;

#define OS_SEM_LOCK(__lock)						\
{												\
	OS_ENTER_CRITICAL_EX((OS_CPU_SR)(*__lock));				\
}


#define OS_SEM_UNLOCK(__lock)					\
{												\
	OS_EXIT_CRITICAL_EX((OS_CPU_SR)(*__lock));				\
}
...
#define OS_SEM_EVENT_INIT(_pSema)			ema_init((_pSema), 1)
...
#define OS_SEM_EVENT_INIT_LOCKED(_pSema) 	sys_sem_new(_pSema, 0)
#define OS_SEM_EVENT_INIT(_pSema)			sys_sem_new(_pSema, 1)
#define OS_SEM_EVENT_DESTORY(_pSema)		sys_sem_free(_pSema)
#define OS_SEM_EVENT_WAIT(_pSema, _status)		(_status) = sys_arch_sem_wait(_pSema,0)
#define OS_SEM_EVENT_UP(_pSema)				sys_sem_signal(_pSema)
...
struct os_cookie {
	...
	RTMP_NET_TASK_STRUCT uapsd_eosp_sent_work; //edit by lck
#else
...
#define get_unaligned(x)							RtmpOsGetUnaligned((UINT16 *)x)
#define get_unaligned32(x)							RtmpOsGetUnaligned32((UINT32 *)x)
#define get_unalignedlong(x)						RtmpOsGetUnaligned32((UINT32 *)x)

#define put_unaligned(x)							RtmpOsPutUnaligned((UINT16 *)x)
#define put_unaligned32(x)							RtmpOsPutUnaligned32((UINT32 *)x)
#define put_unalignedlong(x)						RtmpOsPutUnaligned32((UINT32 *)x)
...
#ifdef MT7601
extern const uint32_t bulkout_ep_map[10];
extern const uint32_t bulkin_ep_map[8];
#define usb_sndbulkpipe(X,Y) (bulkout_ep_map[(uint32_t)(Y) & 0x7f] + 2)
#define usb_rcvbulkpipe(X,Y) (bulkin_ep_map[(uint32_t)(Y) & 0x7f] + 2)
#else
#define usb_sndbulkpipe(X,Y) ((Y&0x7f) + 2)
#define usb_rcvbulkpipe(X,Y) (2)
#endif
...
/* Fill Bulk URB Macro */
/*
extern void usb_fill_bulk_urb(struct urb *urb, struct usb_device *dev, unsigned int
...
inline void usb_fill_bulk_urb(struct urb *urb, struct usb_device *dev, unsigned int
    pipe, void *transfer_buffer, int buffer_length, void *complete_fn, void
    *context)
{
    urb->dev = dev;
