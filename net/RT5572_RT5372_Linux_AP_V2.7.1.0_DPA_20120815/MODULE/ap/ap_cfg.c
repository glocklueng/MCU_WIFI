...
const COUNTRY_CODE_TO_COUNTRY_REGION allCountry[] = {
...
*/
INT	set_if_type(IN	PRTMP_ADAPTER	pAd, IN	INT	arg)
{
	ULONG       i;
	POS_COOKIE 	pObj = (POS_COOKIE) pAd->OS_Cookie;
	UCHAR 		ifIndex;
	...
	pObj->ioctl_if_type = arg;
	pObj->ioctl_if = 0;
	return TRUE;
}


INT get_if_type(IN	PRTMP_ADAPTER	pAd)
{
	ULONG       i;
	POS_COOKIE 	pObj = (POS_COOKIE) pAd->OS_Cookie;
	UCHAR 		ifIndex;
	...
	IN  PSTRING arg)
{
	...
	if(strlen(arg) <= MAX_LEN_OF_SSID)
	{
					...
					pAd->ApCfg.ApCliTab[ifIndex].PSKLen > 0)
		{
			...
			assert(pAd->ApCfg.ApCliTab[ifIndex].PSK);
		#endif
		...
		#if MEMORY_OPTIMIZATION
			mem_free(pAd->ApCfg.ApCliTab[ifIndex].PSK);
			pAd->ApCfg.ApCliTab[ifIndex].PSK = 0;
			pAd->ApCfg.ApCliTab[ifIndex].PSKLen = 0;
		#endif
