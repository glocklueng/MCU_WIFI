	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	for(apidx=0; apidx<pAd->ApCfg.BssidNum; apidx++)
	{
		...
		MakeOutgoingFrame(pOutBuffer,                 &FrameLen,
						  (sizeof(HEADER_802_11)  - 1/*wsum*/),      &ProbeRspHdr,
...
#else
		RTMPConflictSsidDetection(pAd, (PUCHAR)Ssid, SsidLen, (CHAR)Elem->Rssi0, (CHAR)Elem->Rssi1, (CHAR)Elem->Rssi2, Elem->AntSel);
#endif
#endif /* IDS_SUPPORT */
	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	if (MlmeScanReqSanity(pAd, Elem->Msg, Elem->MsgLen, &BssType, (PCHAR)Ssid, &SsidLen, &ScanType))
	{
		...
		BssTabRstSimple(pAd->ScanTabSimple);	//add by lck
...
extern BSS_ENTRY_SIMPLE *BssTableSearchSimple(
...
void fill_scan_result_data(struct scan_result_data *p_data,
							...
							int bss_type,
							int privacy,
							NDIS_802_11_WEP_STATUS	wep_status,
							NDIS_802_11_AUTHENTICATION_MODE	auth_mode)
{

	
	memcpy(p_data->bssid, bssid, MAC_ADDR_LEN);
	
	p_data->channel = channel;
	memcpy(p_data->essid, ssid, MAX_LEN_OF_SSID);
	p_data->essid_len = ssid_len;
	p_data->freq = ieee80211_channel_to_frequency(channel, IEEE80211_BAND_2GHZ);
	p_data->rssi = rssi;
	p_data->mode = (bss_type == BSS_ADHOC)?(MODE_ADHOC):(MODE_AP);
	...
	if(privacy  == 0)
	{
		...
		if(wep_status == Ndis802_11WEPEnabled)
		{
			p_data->proto = WPA_PROTO_OPEN_SHARE;
			p_data->cipher = WPA_CIPHER_WEP104 | WPA_CIPHER_WEP40;
			return;
		}

		//AuthMode
		if(auth_mode == Ndis802_11AuthModeWPA)
		{
			p_data->proto = WPA_PROTO_WPA;
		}else if(auth_mode == Ndis802_11AuthModeWPA2)
		{
			p_data->proto = WPA_PROTO_RSN;
		}else if(auth_mode == Ndis802_11AuthModeWPAPSK)
		{
			p_data->proto = WPA_PROTO_WPA;
		}else if(auth_mode == Ndis802_11AuthModeWPA2PSK)
		{
			p_data->proto = WPA_PROTO_RSN;
		}else{
			p_err("unkown AuthMode:%d\n", auth_mode);
			p_data->proto = WPA_PROTO_NONE;
		}

		//cipher
		if(wep_status == Ndis802_11Encryption2Enabled)
		{
			p_data->cipher = WPA_CIPHER_TKIP;
		}else if(wep_status == Ndis802_11Encryption3Enabled)
		{
			p_data->cipher = WPA_CIPHER_CCMP;
		}else{
			p_err("unkown cipher:%d\n", wep_status);
			p_data->cipher = WPA_CIPHER_NONE;
		}
	}
}
...
*/
								...
								pVIE))
    {
...
#ifdef MT7601
		if (Channel == pAd->CommonCfg.Channel)
			RTMPConflictSsidDetection(pAd, Ssid, SsidLen, Elem->Rssi0, Elem->Rssi1, Elem->Rssi2, Elem->AntSel);
#else
		if (Channel == pAd->CommonCfg.Channel)
		...
		*/
		...
		do{	//ADD by lck
			int found = 0;
			PBSS_ENTRY	pBssEntry_tmp;
			BSS_ENTRY_SIMPLE   *BssEntry;
			pBssEntry_tmp = (PBSS_ENTRY)mem_calloc(sizeof(BSS_ENTRY), 1);
			if(pBssEntry_tmp != 0)
			{
				BssEntrySet(pAd, pBssEntry_tmp, Bssid, Ssid, SsidLen, BssType, BeaconPeriod, &CfParm, AtimWin, 
								CapabilityInfo, SupRate, SupRateLen, ExtRate, ExtRateLen,pHtCapability, pAddHtInfo,HtCapabilityLen, AddHtInfoLen,
								NewExtChannelOffset, Channel, Rssi, TimeStamp, CkipFlag, &EdcaParm, &QosCapability, &QbssLoad, LenVIE, pVIE);

//				RT_CFG80211_SCANNING_INFORM(pAd, Idx, pBssEntry_tmp, Elem->Channel, (UCHAR *)pFrame,
//										Elem->MsgLen, Rssi)
				//ижбз?ииж╠??икии???????
			/*	if(SSID_EQUAL(pAd->MlmeAux.Ssid, pAd->MlmeAux.SsidLen, pBssEntry_tmp->Ssid, pBssEntry_tmp->SsidLen))
				{
					//ScanCnclAction(pAd, 0);
					p_dbg("SSID_EQUAL cancel scan");
					pAd->MlmeAux.Channel = 0;

					pAd->ScanTab.AuthMode = pBssEntry_tmp->AuthMode;
					pAd->ScanTab.WepStatus = pBssEntry_tmp->WepStatus;
					p_dbg("AuthMode:%d, WepStatus:%d", pAd->ScanTab.AuthMode, pAd->ScanTab.WepStatus);
				}*/

				BssEntry = BssTableSearchSimple((BSS_TABLE_SIMPLE*)&pAd->ScanTabSimple, Bssid, Channel);
				...
				if (BssEntry == 0) 
				{
					...
					if(BssEntry)
					{
						...
						fill_scan_result_data(&BssEntry->res_data,
							pBssEntry_tmp->Bssid, 
							pBssEntry_tmp->Ssid, 
							pBssEntry_tmp->SsidLen,
							pBssEntry_tmp->Channel,
							pBssEntry_tmp->Rssi,
							pBssEntry_tmp->BssType,
							pBssEntry_tmp->Privacy,
							pBssEntry_tmp->WepStatus,
							pBssEntry_tmp->AuthMode);
						/*
						p_dbg("%s\n", pBssEntry_tmp->Ssid);
						p_dbg("1WPA.GroupCipher:%d, WPA.bMixMode:%d,WPA.PairCipher:%d, WPA.PairCipherAux:%d, WPA.RsnCapability:%d\n",
							pBssEntry_tmp->WPA.GroupCipher,
							pBssEntry_tmp->WPA.bMixMode,
							pBssEntry_tmp->WPA.PairCipher,
							pBssEntry_tmp->WPA.PairCipherAux,
							pBssEntry_tmp->WPA.RsnCapability);

						p_dbg("2WPA.GroupCipher:%d, WPA.bMixMode:%d,WPA.PairCipher:%d, WPA.PairCipherAux:%d, WPA.RsnCapability:%d\n",
							pBssEntry_tmp->WPA2.GroupCipher,
							pBssEntry_tmp->WPA2.bMixMode,
							pBssEntry_tmp->WPA2.PairCipher,
							pBssEntry_tmp->WPA2.PairCipherAux,
							pBssEntry_tmp->WPA2.RsnCapability);

						p_dbg("AuthMode:%d, WepStatus:%d,Privacy:%d\n",
							pBssEntry_tmp->AuthMode,
							pBssEntry_tmp->WepStatus,
							pBssEntry_tmp->Privacy);
						*/
						//if(pBssEntry_tmp->WPA.GroupCipher = Ndis802_11WEPEnabled;
							
						BssTabInsertSimple((BSS_TABLE_SIMPLE*)&pAd->ScanTabSimple, BssEntry);
					}
				}
			#if MEMORY_OPTIMIZATION
				if(pBssEntry_tmp->VarIEs)
					kfree(pBssEntry_tmp->VarIEs);
			#endif
				kfree(pBssEntry_tmp);
			}
