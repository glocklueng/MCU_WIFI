	...
	*/
	...
	COPY_MAC_ADDR(pApCliEntry->LastBssid, pAd->MlmeAux.Bssid);
			...
			if ((pMacEntry->AuthMode >= Ndis802_11AuthModeWPA) && (pAd->ApCliMlmeAux.VarIELen != 0))
			{
				...
				assert(pAd->ApCliMlmeAux.VarIEs);
				#endif
				...
				#if MEMORY_OPTIMIZATION
				mem_free(pAd->ApCliMlmeAux.VarIEs);
				pAd->ApCliMlmeAux.VarIEs = 0;
...
/*
	...
	==========================================================================
 */
static BOOLEAN ValidApCliEntry(
	IN PRTMP_ADAPTER pAd,
	IN INT apCliIdx)
{
	BOOLEAN result;
	...
	IN  MAC_TABLE_ENTRY *pEntry)
{
	...
	assert(DefaultKeyIdx < SHARE_KEY_NUM);
...
/*	RTMP_OS_NETDEV_OP_HOOK	netDevOpHook; */
	...
	for(apcli_index = 0; apcli_index < MAX_APCLI_NUM; apcli_index++)
	{
		...
		pAd->IoctlIF = IoctlIF;
#endif /* HOSTAPD_SUPPORT */
		RTMP_OS_NETDEV_SET_PRIV(new_dev_p, pAd, INT_APCLI);
