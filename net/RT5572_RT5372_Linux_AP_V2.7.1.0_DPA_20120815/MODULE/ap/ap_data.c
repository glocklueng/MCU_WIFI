...
static BOOLEAN ApAllowToSendPacket(
	IN RTMP_ADAPTER *pAd,
	...
	IN	TX_BLK			*pTxBlk)
{
	...
	{	
		...
		{
			assert(pApCliEntry->DefaultKeyId < SHARE_KEY_NUM);
...
#ifdef DOT11_N_SUPPORT
static VOID APBuildCache802_11Header(
	IN RTMP_ADAPTER		*pAd,
	IN TX_BLK			*pTxBlk,
	IN UCHAR			*pHeader)
{
	...
	*/
	...
	pTxBlk->MpduHeaderLen = (sizeof(HEADER_802_11)  - 1/*wsum*/);
...
static VOID APBuildCommon802_11Header(
	IN  PRTMP_ADAPTER   pAd,
	IN  TX_BLK          *pTxBlk)
{
	...
	*/
	...
	NdisZeroMemory(pHeader_802_11, (sizeof(HEADER_802_11)  - 1/*wsum*/));
...
static PUCHAR AP_Build_ARalink_Frame_Header(
	IN RTMP_ADAPTER *pAd,
	IN TX_BLK		*pTxBlk)
{
...
static PUCHAR AP_Build_AMSDU_Frame_Header(
	IN RTMP_ADAPTER *pAd,
	IN TX_BLK		*pTxBlk)
{
	PUCHAR			pHeaderBufPtr;/*, pSaveBufPtr; */
...
/*	UCHAR			QueIdx = pTxBlk->QueIdx; */
	...
	)
	{
		...
		NdisMoveMemory((PUCHAR)(&pTxBlk->HeaderBuf[TXINFO_SIZE]), (PUCHAR)(&pMacEntry->CachedBuf[0]), TXWISize + (sizeof(HEADER_802_11)  - 1/*wsum*/));
#else
...
/*	PQUEUE_HEADER   pQueue; */
	...
	//mutex_lock(pAd->hard_transmit_lock);
	...
	//mutex_unlock(pAd->hard_transmit_lock);
	return (NDIS_STATUS_SUCCESS);
	...
	IN	RX_BLK			*pRxBlk)
{
	...
	{
		pDA = pHeader->Addr3;
		pSA = (PUCHAR)pHeader + (sizeof(HEADER_802_11)  - 1/*wsum*/);
	}
	...
	IN RX_BLK *pRxBlk)
{
	...
	do
	{
			...
			(pHeader->FC.Wep == 1) && (pRxD->Decrypted == 0))
		{
			...
			assert(pRxWI->KeyIndex < SHARE_KEY_NUM);
		...
		REPORT_MGMT_FRAME_TO_MLME(pAd, pRxWI->WirelessCliID, pHeader,
						pRxWI->MPDUtotalByteCount,
						pRxWI->SNR0, pRxWI->SNR1,
						pRxWI->SNR2, 0, OPMODE_AP);
#endif /* MT7601 */
#ifdef TXBF_SUPPORT
		if (pAd->chipCap.FlgHwTxBfCap)
		{
	...
	IN	UCHAR			FromWhichBSSID)
{
		...
		pAd->ApCfg.MBSSID[pEntry->apidx].IEEE8021X == TRUE))
	{
            ...
            (!MAC_ADDR_EQUAL(pAd->ApCfg.MBSSID[pEntry->apidx].WscControl.EntryAddr, ZERO_MAC_ADDR)))
		{
			...
			if ( IS_MT7601(pAd) )
			{
				REPORT_MGMT_FRAME_TO_MLME(pAd, pRxWI->WIWirelessCliID, pTmpBuf, pRxBlk->DataSize + LENGTH_802_11, pRxWI->RxWISNR2, 0, 0, 0, pRxWI->RxWISNR1, OPMODE_AP);
			}
			else
#endif /* MT7601 */
...
#endif /* DOT1X_SUPPORT */	
	{
		...
		if ( IS_MT7601(pAd) )
		{
			REPORT_MGMT_FRAME_TO_MLME(pAd, pRxWI->WirelessCliID, pTmpBuf, pRxBlk->DataSize + LENGTH_802_11, pRxWI->SNR2, 0, 0, 0, pRxWI->SNR1, OPMODE_AP);
		}
		else
			;
#else
			REPORT_MGMT_FRAME_TO_MLME(pAd, pRxWI->WirelessCliID, pTmpBuf, pRxBlk->DataSize + LENGTH_802_11, pRxWI->SNR0, pRxWI->SNR1, pRxWI->SNR2, 0, OPMODE_AP);
#endif /* MT7601 */	
	}
	...
	IN	UCHAR			FromWhichBSSID)
{
	...
	if (!RTMPCheckWPAframe(pAd, pEntry, pRxBlk->pData, pRxBlk->DataSize, FromWhichBSSID))
	{
		...
		if (!RX_BLK_TEST_FLAG(pRxBlk, fRX_ARALINK))
		{
			...
			//dump_hex("d2", pRxBlk->pData, pRxBlk->DataSize);
	...
	IN	RX_BLK			*pRxBlk)
{
	...
	//dump_hex("rxd", (void*)pRxD, sizeof(RXINFO_STRUC));
	...
	*/
	...
	if ( IS_MT7601(pAd) )
		if (pFmeCtrl->FrDs == 1 && 
			(RTMPReplayAttackDetection(pAd, pHeader->Addr2, pRxWI->RxWISNR2, 0, 0, pRxWI->RxWIBW) == TRUE))
		{
			goto err;
		}
	else
...
#endif /* IKANOS_VX_1X0 */
		//dump_hex("d1", pRxBlk->pData, pRxBlk->DataSize);
...
#ifdef MT7601
		pRxCell = &RxCell;
		pRxPacket = GetPacketFromRxRing(pAd, pRxCell, &bReschedule, &RxPending);
		if (pRxPacket == NULL)
			break;
#else
		pRxCell = &RxCell;
...
//#ifdef MT7601
//		pRxD = (RT28XX_RXD_STRUC *)&(pRxCell->hw_rx_info[0]);
//#else
		pRxD = &(pRxCell->RxD);
//#endif
#ifdef RLT_MAC
		pFceInfo = RxCell.pRxFceInfo;
#endif /* RLT_MAC */
		pData	= GET_OS_PKT_DATAPTR(pRxPacket);
		pRxWI	= (PRXWI_STRUC)pData;
		pHeader = (PHEADER_802_11)(pData + RXWISize);
		//dump_hex("d:", pData, pRxWI->MPDUtotalByteCount);
#ifdef RLT_MAC
		...
		*/
		switch (pHeader->FC.Type)
		{
			case BTYPE_DATA:
				...
				//dump_hex("d:", RxCell.pData, RxCell.DataSize);
	...
	IN PMAC_TABLE_ENTRY pMacEntry)
{
	...
	Length = (sizeof(HEADER_802_11)  - 1/*wsum*/);
