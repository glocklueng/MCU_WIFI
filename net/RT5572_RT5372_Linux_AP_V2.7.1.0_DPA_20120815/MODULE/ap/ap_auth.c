    ...
    IN MLME_QUEUE_ELEM *Elem) 
{
    ...
    if (Elem->Wcid < MAX_LEN_OF_MAC_TABLE)
    {
        ...
        MakeOutgoingFrame(pOutBuffer,				&FrameLen, 
                          (sizeof(HEADER_802_11)  - 1/*wsum*/),	&Hdr, 
                          2,						&pInfo->Reason, 
                          END_OF_ARGS);
	...
	IN MLME_QUEUE_ELEM *Elem)
{
				...
				|| (pAd->ApCfg.MBSSID[apidx].AuthMode == Ndis802_11AuthModeAutoSwitch)))
	{
		...
		if (pEntry)
		{
			...
			if(!pAd->ApMlmeAux.Challenge)
			{
				pAd->ApMlmeAux.Challenge = (CHAR*)mem_malloc(CIPHER_TEXT_LEN);
			}
		#endif
			...
			MgtMacHeaderInit(pAd, &AuthHdr, SUBTYPE_AUTH, 0, 	Addr2, 
								pAd->ApCfg.MBSSID[apidx].Bssid);
			MakeOutgoingFrame(pOutBuffer,            &FrameLen,
								(sizeof(HEADER_802_11)  - 1/*wsum*/), &AuthHdr,
								2,                     &Alg,
								2,                     &Seq,
								2,                     &RspReason,
								1,                     &ChTxtIe,
	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	assert(pAd->ApMlmeAux.Challenge);
