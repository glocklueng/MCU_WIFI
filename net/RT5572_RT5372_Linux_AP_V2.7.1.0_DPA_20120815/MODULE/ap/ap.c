...
/*        if (pAd->CommonCfg.bAPSDCapable == TRUE) */
	...
	*/
	...
	{
		/* DPD-Calibration */
		AndesCalibrationOP(pAd, ANDES_CALIBRATION_DPD, pAd->chipCap.TxALCData.CurrentTemperature);
	...
	for (MacIdx=0; MacIdx < MAX_LEN_OF_MAC_TABLE; MacIdx++)
	{
		...
		if ((Matched == FALSE) && (pAd->ApCfg.MBSSID[Apidx].AccessControlList.Policy == 1))
		{
			...
			if (pAd->MacTab.Content[MacIdx].Sst == SST_ASSOC)
			{
				...
				MakeOutgoingFrame(pOutBuffer, &FrameLen, (sizeof(HEADER_802_11)  - 1/*wsum*/), &DisassocHdr, 2, &Reason, END_OF_ARGS);
				MiniportMMRequest(pAd, 0, pOutBuffer, FrameLen);
				MlmeFreeMemory(pAd, pOutBuffer);
