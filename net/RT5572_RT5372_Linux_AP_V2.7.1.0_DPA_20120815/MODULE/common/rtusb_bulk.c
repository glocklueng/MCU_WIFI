...
const UCHAR	EpToQueue[6]={FIFO_EDCA, FIFO_EDCA, FIFO_EDCA, FIFO_EDCA, FIFO_EDCA, FIFO_MGMT};

/*static BOOLEAN SingleBulkOut = FALSE;*/

VOID	RTUSBInitNullTxDesc(
	IN	PRTMP_ADAPTER	pAd,
	IN	PTX_NULL_CONTEXT		pTxContext,
...
const unsigned char test_pkg[] = {
0x70, 0x00, 0x00, 0x04, 0x00, 0x00, 0x07, 0x40, 0x01, 0x01, 0x5e, 0x70, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x88, 
0x41, 0x00, 0x00, 0x6c, 0xe8, 0x73, 0x2d, 0xb2, 0x9e, 0x94, 0x44, 0x52, 0x34, 0xb8, 0xe3, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, /*0x08, 0x60, 0x6e, 0x81, 0x4f, 0x99, */0x30, 
0x00, 0x00, 0x00, 0x00, 0x00, 0xaa, 0xaa, 0x03, 0x00, 0x00, 0x00, 0x08, 0x00, 0x45, 0x00, 0x00, 0x3c, 0x61, 0x8a, 0x00, 0x00, 0xff, 
0x01, 0xd6, 0x78, 0xc0, 0xa8, 0x01, 0x08, 0xc0, 0xa8, 0x01, 0x65, 0x00, 0x00, 0x55, 0x5a, 0x00, 0x01, 0x00, 0x01, 0x61, 0x62, 0x63, 
...
DECLARE_MONITOR_ITEM("bulk out data cnt", bulk_out_data_cnt) ;
DECLARE_MONITOR_ITEM("bulk out data complite cnt", bulk_out_data_complite_cnt);
	...
	IN	UCHAR			Index)
{
	...
	mutex_lock(pAd->data_bulk_out_lock);
#endif
...
#if USE_SIMPLE_TRANSMIT
		mutex_unlock(pAd->data_bulk_out_lock);
#endif
		return;
	}
...
*/
	...
	*/	
	...
	//memcpy(&pHTTXContext->TransferBuffer->field.WirelessPacket[pHTTXContext->NextBulkOutPosition], test_pkg, 16);
	/* Init Tx context descriptor*/
	RTUSBInitHTTxDesc(pAd, pHTTXContext, BulkOutPipeId, ThisBulkSize, RtmpUsbBulkOutDataPacketComplete);

	//dump_hex("b", &pHTTXContext->TransferBuffer->field.WirelessPacket[pHTTXContext->NextBulkOutPosition], ThisBulkSize);
	pUrb = pHTTXContext->pUrb;
#if USE_SIMPLE_TRANSMIT
	//clear_wait_event(pAd->data_bulk_out_wait);
#endif	
	INC_MONITOR_ITEM_VALUE(bulk_out_data_cnt);
...
#if USE_SIMPLE_TRANSMIT
	//wait_event_timeout(pAd->data_bulk_out_wait, 5000);
#endif
	BULK_OUT_LOCK(&pAd->BulkOutLock[BulkOutPipeId], IrqFlags);
...
USBHST_STATUS RTUSBBulkOutDataPacketComplete(URBCompleteStatus Status, purbb_t pURB, pregs *pt_regs)
{
	...
	INC_MONITOR_ITEM_VALUE(bulk_out_data_complite_cnt);
#if USE_SIMPLE_TRANSMIT
	//wake_up(pAd->data_bulk_out_wait);
#endif
}
...
VOID	RTUSBBulkOutNullFrame(
	IN	PRTMP_ADAPTER	pAd)
{
	PTX_NULL_CONTEXT		pNullContext = &(pAd->NullContext);
	PURB			pUrb;
	int				ret = 0;
	...
	/* Init Tx context descriptor*/
	RTUSBInitNullTxDesc(pAd, pNullContext, 0, (void*)RtmpUsbBulkOutNullFrameComplete);
...
USBHST_STATUS RTUSBBulkOutNullFrameComplete(URBCompleteStatus Status1, purbb_t pURB, pregs *pt_regs)
{
	PRTMP_ADAPTER		pAd;
	...
	pNullContext	= (PTX_NULL_CONTEXT)RTMP_OS_USB_CONTEXT_GET(pURB);
	...
	IN	UCHAR			Index)
{
	...
	/* Init Tx context descriptor*/
	RTUSBInitTxDesc(pAd, pMLMEContext, MGMTPIPEIDX, (void*)RtmpUsbBulkOutMLMEPacketComplete);
...
USBHST_STATUS RTUSBBulkOutMLMEPacketComplete(URBCompleteStatus Status1, purbb_t pURB, pregs *pt_regs)
{
	PTX_CONTEXT			pMLMEContext;
	PRTMP_ADAPTER		pAd;
	NTSTATUS			Status;
...
VOID	RTUSBBulkOutPsPoll(
	IN	PRTMP_ADAPTER	pAd)
{
	PTX_NULL_CONTEXT		pPsPollContext = &(pAd->PsPollContext);
	PURB			pUrb;
	int				ret = 0;
	unsigned long	IrqFlags;
	...
	/* Init Tx context descriptor*/
	RTUSBInitNullTxDesc(pAd, pPsPollContext, MGMTPIPEIDX, (void*)RtmpUsbBulkOutPsPollComplete);
...
USBHST_STATUS RTUSBBulkOutPsPollComplete(URBCompleteStatus Status1, purbb_t pURB, pregs *pt_regs)
{
	PRTMP_ADAPTER		pAd;
	...
	pPsPollContext= (PTX_NULL_CONTEXT)RTMP_OS_USB_CONTEXT_GET(pURB);
