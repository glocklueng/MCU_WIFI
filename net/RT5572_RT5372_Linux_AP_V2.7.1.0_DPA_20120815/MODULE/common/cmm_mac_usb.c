	...
	IN	PRTMP_ADAPTER	pAd)
{
	...
	for (i = 0; i < (RX_RING_SIZE); i++)
	{
		...
		/*Allocate URB*/
		pRxContext->pUrb = (PURB)RTUSB_ALLOC_URB(0);		
		...
		pRxContext->TransferBuffer = (PUCHAR)RTUSB_URB_ALLOC_BUFFER(pObj->pUsb_Dev, MAX_RXBULK_SIZE, &pRxContext->data_dma);
	...
	pCmdRspEventContext->pUrb = (PURB)RTUSB_ALLOC_URB(0);
	pCmdRspEventContext->CmdRspBuffer = (PUCHAR)mem_calloc(CMD_RSP_BULK_SIZE, 1);
...
========================================================================
*/
HTTX_BUFFER bulk_out_tx_buffer __attribute__((__aligned__(4)));

TX_NULL_BUFFER bulk_out_tx_null_buffer __attribute__((__aligned__(4)));
TX_NULL_BUFFER bulk_out_tx_poll_buffer __attribute__((__aligned__(4)));
	...
	IN	PRTMP_ADAPTER	pAd)
{
	...
	pAd->data_bulk_out_wait = init_event();
	pAd->data_bulk_out_lock = mutex_init();
#endif
	pAd->hard_transmit_lock = mutex_init();
	pAd->mlme_transmit_lock = mutex_init();
	pAd->deque_transmit_lock = mutex_init();
...
/*
			...
			if(acidx == 0)
			{
				pHTTXContext->pUrb = (PURB)RTUSB_ALLOC_URB(0);
				pHTTXContext->TransferBuffer = &bulk_out_tx_buffer;
				NdisZeroMemory(pHTTXContext->TransferBuffer->Aggregation, 4);
				
			}else{
				pHTTXContext->pUrb = 0;
				pHTTXContext->TransferBuffer = 0;
			}
			pHTTXContext->pAd = pAd;
		...
		for (i = 0; i < MGMT_RING_SIZE; i++)
		{
			...
			/*Allocate URB for MLMEContext*/
			pMLMEContext = (PTX_CONTEXT) pAd->MgmtRing.Cell[i].AllocVa;
			pMLMEContext->pUrb = (purbb_t)RTUSB_ALLOC_URB(0);
			if (pMLMEContext->pUrb == NULL)
			{
				DBGPRINT(RT_DEBUG_ERROR, ("<-- ERROR in Alloc TX MLMEContext[%d] urb!! \n", i));
				Status = NDIS_STATUS_RESOURCES;
		...
		pNullContext->pUrb = (PURB)RTUSB_ALLOC_URB(0);
		pNullContext->TransferBuffer = &bulk_out_tx_null_buffer;
...
/*		
		...
		if (Status != NDIS_STATUS_SUCCESS)
			goto err;
*/
		pPsPollContext->pUrb = (PURB)RTUSB_ALLOC_URB(0);
		pPsPollContext->TransferBuffer = &bulk_out_tx_poll_buffer;
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	if ( IS_MT7601(pAd) )
	{
		...
		AndesPwrSavingOP(pAd, RADIO_OFF, 0x04, 0, 0, 0, 0);
	}
	else
#endif /* MT7601 */
	{
