...
const UINT8 WPS_DH_G_VALUE[1] = {0x02};
const UINT8 WPS_DH_P_VALUE[192] = 
{
...
/* General used field */
const UCHAR	STA_Wsc_Pri_Dev_Type[8] = {0x00, 0x01, 0x00, 0x50, 0xf2, 0x04, 0x00, 0x01};

#ifdef CONFIG_AP_SUPPORT
const UCHAR	AP_Wsc_Pri_Dev_Type[8] = {0x00, 0x06, 0x00, 0x50, 0xf2, 0x04, 0x00, 0x01};
		...
		case WSC_MSG_EAP_REQ_START:
			...
			if (pWscControl->RegData.ReComputePke == 1)
			{
            	...
            	RT_DH_PublicKey_Generate (
                    (UINT8*)WPS_DH_G_VALUE, sizeof(WPS_DH_G_VALUE),
            	    (UINT8*)WPS_DH_P_VALUE, sizeof(WPS_DH_P_VALUE),
    ...
    IN  BOOLEAN         bEnrollee)
{
	...
	CurApIdx = (pWscControl->EntryIfIdx & 0x0F);
	{
		...
		if (pCredential->EncrType != WSC_ENCRTYPE_NONE)
		{
				...
				WepKeyId = (pCredential->KeyIndex - 1); /* KeyIndex = 1 ~ 4 */
				if ((WepKeyId >= 0) && (WepKeyId < SHARE_KEY_NUM))
				{
    ...
    IN ULONG Context)
{
	...
	while (pTask && !RTMP_OS_TASK_IS_KILLED(pTask))
	{
		...
		if (pAd->pWscElme && (pAd->pWscElme->MsgLen != 0))
		{
			...
			MLME_QUEUE_ELEM	*pElme = alloc_mlme_elem(0);

			if (pElme)
			{
				int irq_flag;   //edit by lck
				/*
				...
				cpy_mlme_elem(pElme, pAd->pWscElme);
			#if MLMEQUEUE_ALLOC_INUSE
				mem_free(pAd->pWscElme->Msg);
				pAd->pWscElme->Msg = NULL;
				pAd->pWscElme->inuse_len = 0;
			#endif
				local_irq_restore(irq_flag);
			
				WpsSmProcess(pAd, pElme);
				//os_free_mem(NULL, pElme);
				free_mlme_elem(pElme);
			}
		}
...
#define RT_WPS_STK_SIZE		512

static OS_STK RTWPSThreadSTK[RT_WPS_STK_SIZE];
...
NDIS_STATUS WscThreadInit(RTMP_ADAPTER *pAd)
{
	...
	status = RtmpOSTaskAttach(pTask, wsc_write_dat_file_thread, (ULONG)&pAd->wscTask, 
		RTWPSThreadSTK, RT_WPS_STK_SIZE, TASK_WPS_PRIO);
	...
	if (status == NDIS_STATUS_SUCCESS)
	{
		...
		pAd->pWscElme = alloc_mlme_elem(0);
	}
