...
const struct _MATProtoOps MATProtoIPHandle =
{
	MATProto_IP_Init,
	MATProto_IP_Tx,
	MATProto_IP_Rx,
...
const struct _MATProtoOps MATProtoARPHandle =
{
	MATProto_ARP_Init,
	MATProto_ARP_Tx,
	MATProto_ARP_Rx,
...
static  NDIS_STATUS getDstIPFromIpPkt(
	IN PUCHAR pIpHdr, 
	IN UINT *dstIP)
{
...
static  NDIS_STATUS getSrcIPFromIpPkt(
	IN PUCHAR pIpHdr,
	IN UINT   *pSrcIP)
{
...
/*dumpPkt(RTPKT_TO_OSPKT(pSkb)->data, RTPKT_TO_OSPKT(pSkb)->len); */
	...
	*/
	...
	if (pRealMac == NULL)
		pRealMac = (PUCHAR)&BROADCAST_ADDR[0];
...
const static UCHAR  DHCP_MAGIC[]= {0x63, 0x82, 0x53, 0x63};
...
static void IPintToIPstr(int ipint, char Ipstr[20], ULONG BufLen)
{
	 int temp = 0;
