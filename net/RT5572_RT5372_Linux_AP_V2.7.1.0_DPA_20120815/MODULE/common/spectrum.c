	...
	IN UINT8 channel)
{
	...
	if (strncmp(pCountry, "US", 2) == 0)
	{
		...
		pRegulatoryClass = (DOT11_REGULATORY_INFORMATION*)&USARegulatoryInfo[0];
	}
	else if (strncmp(pCountry, "JP", 2) == 0)
	{
		MaxRegulatoryClassNum = JP_REGULATORY_INFO_SIZE;
		pRegulatoryClass = (DOT11_REGULATORY_INFORMATION*)&JapanRegulatoryInfo[0];
	}
	else
	{
	...
	IN UINT8 RegulatoryClass)
{
	...
	if (strncmp(pCountry, "US", 2) == 0)
	{
		...
		pChannelSet = (DOT11_CHANNEL_SET*)&USARegulatoryInfo[RegulatoryClass].ChannelSet;
	}
	else if (strncmp(pCountry, "JP", 2) == 0)
	{
		...
		pChannelSet = (DOT11_CHANNEL_SET*)&JapanRegulatoryInfo[RegulatoryClass].ChannelSet;
	}
	else
	{
		DBGPRINT(RT_DEBUG_ERROR, ("%s: Unknow Country (%s)\n",
	...
	IN PMEASURE_REQ_INFO pMeasureReqIE)
{
	...
	MakeOutgoingFrame(pFrameBuf,					&TempLen,
						1,							&ElementID,
						1,							&Len,
						sizeof(MEASURE_REQ_INFO) - 1/*wsum*/,	pMeasureReqIE,
						END_OF_ARGS);
	...
	IN PUINT8 pReportInfo)
{
	...
	Len = sizeof(MEASURE_REPORT_INFO) - 1/*wsum*/ + ReportLnfoLen;
	...
	IN UINT16 NumOfRepetitions)
{
	...
	NdisZeroMemory(&MeasureReqIE, sizeof(MEASURE_REQ_INFO) - 1/*wsum*/);
	...
	IN PUINT8 pReportInfo)
{
	...
	NdisMoveMemory(pOutBuffer, (PCHAR)&ActHdr, (sizeof(HEADER_802_11)  - 1/*wsum*/));
	FrameLen = (sizeof(HEADER_802_11)  - 1/*wsum*/);
	...
	// prepare Measurement IE.
	NdisZeroMemory(&MeasureRepIE, sizeof(MEASURE_REPORT_INFO) - 1/*wsum*/);
	...
	OUT PCH_SW_ANN_INFO pChSwAnnInfo)
{
	...
	/* skip 802.11 header.*/
	MsgLen -= (sizeof(HEADER_802_11)  - 1/*wsum*/);
		...
		switch(eid_ptr->Eid)
		{
			case IE_MEASUREMENT_REPORT:
				...
				if (pMeasureReportInfo->ReportType == RM_BASIC)
				{
					...
					NdisMoveMemory((void*)&pReport->MeasureStartTime, ptr + 1, 8);
					NdisMoveMemory((void*)&pReport->MeasureDuration, ptr + 9, 2);
...
/*	if ((pMeasureReportInfo = kmalloc(sizeof(MEASURE_RPI_REPORT), GFP_ATOMIC)) == NULL)*/
	...
	NdisZeroMemory(&MeasureReportInfo, sizeof(MEASURE_REPORT_INFO) - 1/*wsum*/);
	...
	IN	PSTRING			arg)
{
	...
	TotalLen = sizeof(MEASURE_REQ_INFO) - 1/*wsum*/ + sizeof(MEASURE_REQ);

	MakeMeasurementReqFrame(pAd, pOutBuffer, &FrameLen,
		sizeof(MEASURE_REQ_INFO) - 1/*wsum*/, CATEGORY_RM, RM_BASIC,
	...
	do
	{
		if (strncmp(pCountryCode, "US", 2) == 0)
		{
			if (RugClass >= USA_REGULATORY_INFO_SIZE)
				break;
			pRugClass = (DOT11_REGULATORY_INFORMATION*)&USARegulatoryInfo[RugClass];
		}
		...
		if (strncmp(pCountryCode, "JP", 2) == 0)
		{
			if (RugClass >= JP_REGULATORY_INFO_SIZE)
				break;
			pRugClass = (DOT11_REGULATORY_INFORMATION*)&JapanRegulatoryInfo[RugClass];
