...
const UCHAR	SNAP_802_1H[] = {0xaa, 0xaa, 0x03, 0x00, 0x00, 0x00};
const UCHAR	SNAP_BRIDGE_TUNNEL[] = {0xaa, 0xaa, 0x03, 0x00, 0x00, 0xf8};
const UCHAR	EAPOL[] = {0x88, 0x8e};
const UCHAR   TPID[] = {0x81, 0x00}; /* VLAN related */

const UCHAR	IPX[] = {0x81, 0x37};
const UCHAR	APPLE_TALK[] = {0x80, 0xf3};

const UCHAR	 OfdmRateToRxwiMCS[12] = {
	0,  0,	0,  0,
	0,  1,	2,  3,	/* OFDM rate 6,9,12,18 = rxwi mcs 0,1,2,3 */
	4,  5,	6,  7,	/* OFDM rate 24,36,48,54 = rxwi mcs 4,5,6,7 */
};
const UCHAR	 RxwiMCSToOfdmRate[12] = {
	RATE_6,  RATE_9,	RATE_12,  RATE_18,
	RATE_24,  RATE_36,	RATE_48,  RATE_54,	/* OFDM rate 6,9,12,18 = rxwi mcs 0,1,2,3 */
	4,  5,	6,  7,	/* OFDM rate 24,36,48,54 = rxwi mcs 4,5,6,7 */
};

const UCHAR MapUserPriorityToAccessCategory[8] = {QID_AC_BE, QID_AC_BK, QID_AC_BK, QID_AC_BE, QID_AC_VI, QID_AC_VI, QID_AC_VO, QID_AC_VO};
	...
	IN	UINT			Length)
{
	...
	UCHAR *rtmpHwHdr; /*RTMP_HW_HDR_LEN];*/
	BOOLEAN			bUseDataQ = FALSE, FlgDataQForce = FALSE, FlgIsLocked = FALSE;
	int 			retryCnt = 0;

	rtmpHwHdr = (UCHAR*)mem_malloc(TXINFO_SIZE +  pAd->chipCap.TXWISize);
	if(!rtmpHwHdr)
	...
	IN	PNDIS_PACKET	pPacket)
{
	...
	//mutex_lock(pAd->mlme_transmit_lock);
	...
	IN  UCHAR           Max_Tx_Packets)
{
	...
	pTxBlk = (TX_BLK*)mem_malloc(sizeof(TX_BLK));
	if(!pTxBlk)
		return;
...
/*    UCHAR			IrqState;*/
	...
	BssTabFlushSimple(&pAd->ScanTabSimple); //add  by lck
...
*/
	...
	//if(sys_cfg.scan_wait)
	//	wake_up(sys_cfg.scan_wait);
	//add by lck
	#if 0
	if(pAd->connect_by_bssentry_ready){
		pAd->connect_by_bssentry_ready = FALSE;
		p_err("connect_by_bssentry");
		send_cmd_connect_by_bssentry();
	}else if(pAd->connect_by_bssentry_pending){
		
		if(--pAd->connect_by_bssentry_retry_cnt > 0)
		{
			BOOLEAN        Cancelled;
...
void ZERO_MAC_TABLE(MAC_TABLE_ENTRY *pEntry)
{
	UCHAR *p_rsn;
	...
	if(pEntry->RSN_IE){
		p_rsn = pEntry->RSN_IE;
		memset(pEntry->RSN_IE, 0, pEntry->RSNIE_Len);
	}else{
		p_rsn = (UCHAR*)mem_calloc(MAX_LEN_OF_RSNIE, 1);
	}
			...
			IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
			{
				...
				if (pAd->MacTab.Content[i].Sst == SST_ASSOC)
				{
			    	...
			    	MakeOutgoingFrame(pOutBuffer,            &FrameLen,
			    	                  (sizeof(HEADER_802_11)  - 1/*wsum*/), &DeAuthHdr,
			    	                  2,                     &Reason,
			    	                  END_OF_ARGS);
	...
	IN PRXWI_STRUC  pRxWI)
{
	...
	{
#ifdef MT7601
		pRssi->LastRssi0 = ConvertToRssi(pAd, (CHAR)rssi0, RSSI_0, pRxWI->SNR1, pRxWI->BW);
#else
	...
	{   
#ifdef MT7601
		pRssi->LastRssi1 = ConvertToRssi(pAd, (CHAR)rssi1, RSSI_1, pRxWI->SNR1, pRxWI->BW);
#else
	...
	{
#ifdef MT7601
		pRssi->LastRssi2 = ConvertToRssi(pAd, (CHAR)rssi2, RSSI_2, pRxWI->SNR1, pRxWI->BW);
#else
...
#define REORDERING_PACKET_TIMEOUT		((100 * OS_HZ)/1000)	/* system ticks -- 100 ms*/
	...
	RT_80211_TO_8023_PACKET(pAd, VLAN_VID, VLAN_Priority,
							pRxBlk, Header802_3, FromWhichBSSID, (UCHAR*)TPID);
	...
	IN	UCHAR			FromWhichBSSID)
{
	...
	if (VLAN_VID == 0) /* maybe WDS packet */
		...
		RT_VLAN_PKT_DUPLICATE(pPacket2, pAd, VLAN_VID, VLAN_Priority,
							...
							Payload2Size, FromWhichBSSID, (UCHAR*)TPID);
	}
#endif /* CONFIG_AP_SUPPORT */
