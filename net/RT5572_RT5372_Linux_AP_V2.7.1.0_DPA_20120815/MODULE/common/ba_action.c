 ...
 */
static  void ba_enqueue(struct reordering_list *list, struct reordering_mpdu *mpdu_blk)
{
	list->qlen++;
 ...
 * caller lock critical section if necessary
 */
static  struct reordering_mpdu * ba_dequeue(struct reordering_list *list)
{
...
static  struct reordering_mpdu  *ba_reordering_mpdu_dequeue(struct reordering_list *list)
{
	return(ba_dequeue(list));
}


static  struct reordering_mpdu  *ba_reordering_mpdu_probe(struct reordering_list *list)
	{
	ASSERT(list);
	...
	if (pBAEntry != NULL)
	{
		...
		else
		{
			...
			RTMPInitTimer(pAd, &pBAEntry->RECBATimer, GET_TIMER_FUNCTION(BARecSessionIdleTimeout), pBAEntry, TRUE);
                        p_dbg("alloc RECBATimer:%x", pBAEntry->RECBATimer.TimerObj);
                }
...
MLME_QUEUE_ELEM *alloc_mlme_elem(int msg_size)
{
	...
	if(msg_size)
	{
		os_alloc_mem(NULL, (UCHAR **)&Elem->Msg, msg_size);
		if(!Elem->Msg){
			mem_free(Elem);
			return 0;
		}
	}
	Elem->inuse_len = msg_size;
	memset(Elem->Msg, 0, msg_size);
#endif
	return Elem;
}
...
{
#if MLMEQUEUE_ALLOC_INUSE
	UCHAR  *save_msg = to->Msg;
	int save_msg_len = to->inuse_len;
#endif
	memcpy(to, from, sizeof(MLME_QUEUE_ELEM));	
#if MLMEQUEUE_ALLOC_INUSE
	to->Msg = save_msg;
	to->inuse_len = save_msg_len;
	if(from->inuse_len <= to->inuse_len){
		memcpy(to->Msg, from->Msg, from->inuse_len);
	}else{
		p_err("need realloc mlme msg:%d,%d", from->inuse_len, to->inuse_len);
		mem_free(to->Msg);
		os_alloc_mem(NULL, (UCHAR **)&to->Msg, from->inuse_len);
		memcpy(to->Msg, from->Msg, from->inuse_len);
		to->inuse_len = from->inuse_len;
	}
#endif
}
						 ...
						 IN      BOOLEAN         bForceSend)
{
	...
	if ((Idx == 0) || (Idx >= MAX_LEN_OF_BA_ORI_TABLE))
	{
		if (bForceSend == TRUE)
		{
			...
			//os_alloc_mem(NULL, (UCHAR **)&Elem, sizeof(MLME_QUEUE_ELEM));
			MLME_QUEUE_ELEM *Elem =	alloc_mlme_elem(sizeof(DelbaReq));
						 ...
						 IN      BOOLEAN         bPassive)
{
	...
	if ((TID == pBAEntry->TID) && (pBAEntry->REC_BA_Status == Recipient_Accept))
	{
		...
		/*ULONG   offset; */
		/*UINT32  VALUE;*/
	        p_dbg("Cancel RECBATimer:%x", pBAEntry->RECBATimer.TimerObj);	
		RTMPCancelTimer(&pBAEntry->RECBATimer, &Cancelled);         
...

{
	...
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
	{
		...
		if (IS_ENTRY_APCLI(&pAd->MacTab.Content[Elem->Wcid]))
		{
			...
			ActHeaderInit(pAd, (PHEADER_802_11)&ADDframe.Hdr, pAddr, pAd->ApCfg.ApCliTab[apidx].CurrentAddress, pAddr);		
		}
		else
#endif /* APCLI_SUPPORT */
#ifdef WDS_SUPPORT
		if (IS_ENTRY_WDS(&pAd->MacTab.Content[Elem->Wcid]))
		{
			...
			ActHeaderInit(pAd, (PHEADER_802_11)&ADDframe.Hdr, pAddr, pAd->ApCfg.MBSSID[MAIN_MBSSID].Bssid, pAddr);		
		}
		else
#endif /* WDS_SUPPORT */
		{
			apidx = pAd->MacTab.Content[Elem->Wcid].apidx;
			ActHeaderInit(pAd, (PHEADER_802_11)&ADDframe.Hdr, pAddr, pAd->ApCfg.MBSSID[apidx].Bssid, pAd->ApCfg.MBSSID[apidx].Bssid);
		}
	}
#endif /* CONFIG_AP_SUPPORT */
				   ...
				   IN UCHAR				Psmp) 
{
	...
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
	{
		...
		if (IS_ENTRY_APCLI(&pAd->MacTab.Content[Wcid]))
		{
			...
			ActHeaderInit(pAd, (PHEADER_802_11)&Frame.Hdr, pAd->MacTab.Content[Wcid].Addr, pAd->ApCfg.ApCliTab[apidx].CurrentAddress, pAd->MacTab.Content[Wcid].Addr);		
		}
		else
#endif /* APCLI_SUPPORT */
#ifdef WDS_SUPPORT
		if (IS_ENTRY_WDS(&pAd->MacTab.Content[Wcid]))
		{
			...
			ActHeaderInit(pAd, (PHEADER_802_11)&Frame.Hdr, pAd->MacTab.Content[Wcid].Addr, pAd->ApCfg.MBSSID[MAIN_MBSSID].Bssid, pAd->MacTab.Content[Wcid].Addr);
		}
		else
#endif /* WDS_SUPPORT */
		{
			apidx = pAd->MacTab.Content[Wcid].apidx;
			ActHeaderInit(pAd, (PHEADER_802_11)&Frame.Hdr, pAd->MacTab.Content[Wcid].Addr, pAd->ApCfg.MBSSID[apidx].Bssid, pAd->ApCfg.MBSSID[apidx].Bssid);
		}
	}
#endif /* CONFIG_AP_SUPPORT */
				   ...
				   IN UCHAR				Wcid)
{
	...
	MakeOutgoingFrame(pOutBuffer,               &FrameLen,
					  sizeof(FRAME_RM_REQ_ACTION) - 1 /*wsum*/,      &Frame,
...
*/
	...
	if (!RX_BLK_TEST_FLAG(pRxBlk, fRX_AMSDU))
	{
		...
		IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
		{
			...
			data_p = (UCHAR*)OS_PKT_HEAD_BUF_EXTEND(pRxPkt, LENGTH_802_3+VLAN_Size);
			RT_VLAN_8023_HEADER_COPY(pAd, VLAN_VID, VLAN_Priority,
									Header802_3, LENGTH_802_3,
									data_p, FromWhichBSSID, (UCHAR*)TPID);
		}
#endif /* CONFIG_AP_SUPPORT */
