...
//			NdisMoveMemory(pAd->UsbVendorReqBuf, TransferBuffer, TransferBufferLength);
		...
		do {
			...
			RTUSB_CONTROL_MSG(pObj->pUsb_Dev, 0, Request, RequestType, Value, Index, TransferBuffer, TransferBufferLength, CONTROL_TIMEOUT_JIFFIES, RET);
	...
	else
	{
		...
		else
		{
			...
			if (pAd->bulkResetPipeid == 0)
			{
				...
				PTX_NULL_CONTEXT pNULLContext = (PTX_NULL_CONTEXT)(&pAd->NullContext);
				PTX_NULL_CONTEXT pPsPollContext = (PTX_NULL_CONTEXT)(&pAd->PsPollContext);
...
static NTSTATUS ResetBulkInHdlr(IN PRTMP_ADAPTER pAd, IN PCmdQElmt CMDQelmt)
{
...
#endif /* RALINK_ATE */
	{
		...
		{
			DBGPRINT_RAW(RT_DEBUG_ERROR, ("BulkIn IRP Pending!!!,%d\n", pAd->PendingRx));
...
static NTSTATUS CmdRspEventCallback(IN PRTMP_ADAPTER pAd, IN PCmdQElmt CMDQelmt)
{
	...
	(*CmdRspHandlerTable[pFceInfo->evt_type])(
		pAd, 
		((char*)CMDQelmt->buffer) + sizeof(*pFceInfo));
...
const static CMDHdlr CMDHdlrTable[] = {
...
static BOOLEAN ValidCMD(IN PCmdQElmt CMDQelmt)
{
		...
		if (pAd->BulkOutPending[idx] == TRUE)
		{
				...
				)
			{
				...
				else if (idx == MGMTPIPEIDX)
				{
					...
					PTX_NULL_CONTEXT pNULLContext, pPsPollContext;
					
					//Check MgmtContext.
					pMLMEContext = (PTX_CONTEXT)(pAd->MgmtRing.Cell[pAd->MgmtRing.TxDmaIdx].AllocVa);
					pPsPollContext = (PTX_NULL_CONTEXT)(&pAd->PsPollContext);
					pNULLContext = (PTX_NULL_CONTEXT)(&pAd->NullContext);
