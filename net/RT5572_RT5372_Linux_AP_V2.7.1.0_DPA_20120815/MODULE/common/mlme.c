...
const UCHAR	CISCO_OUI[] = {0x00, 0x40, 0x96};

const UCHAR   RALINK_OUI[]  = {0x00, 0x0c, 0x43};
const UCHAR	WPA_OUI[] = {0x00, 0x50, 0xf2, 0x01};
const UCHAR	RSN_OUI[] = {0x00, 0x0f, 0xac};
const UCHAR	WAPI_OUI[] = {0x00, 0x14, 0x72};
const UCHAR   WME_INFO_ELEM[]  = {0x00, 0x50, 0xf2, 0x02, 0x00, 0x01};
const UCHAR   WME_PARM_ELEM[] = {0x00, 0x50, 0xf2, 0x02, 0x01, 0x01};
const UCHAR   BROADCOM_OUI[]  = {0x00, 0x90, 0x4c};
const UCHAR   WPS_OUI[] = {0x00, 0x50, 0xf2, 0x04};
...
const ULONG BasicRateMask[12]				= {0xfffff001 /* 1-Mbps */, 0xfffff003 /* 2 Mbps */, 0xfffff007 /* 5.5 */, 0xfffff00f /* 11 */,
									  0xfffff01f /* 6 */	 , 0xfffff03f /* 9 */	  , 0xfffff07f /* 12 */ , 0xfffff0ff /* 18 */,
									  0xfffff1ff /* 24 */	 , 0xfffff3ff /* 36 */	  , 0xfffff7ff /* 48 */ , 0xffffffff /* 54 */};

const UCHAR BROADCAST_ADDR[MAC_ADDR_LEN] = {0xff, 0xff, 0xff, 0xff, 0xff, 0xff};
const UCHAR ZERO_MAC_ADDR[MAC_ADDR_LEN]  = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
const UCHAR  RateIdToMbps[]	 = { 1, 2, 5, 11, 6, 9, 12, 18, 24, 36, 48, 54, 72, 100};
const USHORT RateIdTo500Kbps[] = { 2, 4, 11, 22, 12, 18, 24, 36, 48, 72, 96, 108, 144, 200};
...
const UCHAR  NewExtChanIe = IE_SECONDARY_CH_OFFSET;
const UCHAR  BssCoexistIe = IE_2040_BSS_COEXIST;
const UCHAR  ExtHtCapIe = IE_EXT_CAPABILITY;
#endif /* DOT11_N_SUPPORT */
const UCHAR  ExtCapIe = IE_EXT_CAPABILITY;
const UCHAR  ErpIe	 = IE_ERP;
const UCHAR  DsIe 	 = IE_DS_PARM;
const UCHAR  TimIe	 = IE_TIM;
const UCHAR  WpaIe	 = IE_WPA;
const UCHAR  Wpa2Ie	 = IE_WPA2;
const UCHAR  IbssIe	 = IE_IBSS_PARM;
const UCHAR  WapiIe	 = IE_WAPI;

extern const UCHAR	WPA_OUI[];

const UCHAR	SES_OUI[] = {0x00, 0x90, 0x4c};

const UCHAR	ZeroSsid[32] = {0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
			...
			switch (Elem->Machine) 
			{
                ...
                case WSC_STATE_MACHINE:
					if (pAd->pWscElme)
					{
						...
						cpy_mlme_elem(pAd->pWscElme, Elem);
	...
	IN UCHAR	 Channel) 
{
	...
	p_entry = Tab->BssEntry;
	do{
		if ((((p_entry->res_data.channel <= 14) && (Channel <= 14)) ||
			 ((p_entry->res_data.channel > 14) && (Channel > 14))) &&
			MAC_ADDR_EQUAL(p_entry->res_data.bssid, pBssid)) 
		{ 
			local_irq_restore(irq_flag);
			return p_entry;
		}
	...
	IN BSS_TABLE *Tab) 
{
	...
	for (i = 0; i < MAX_LEN_OF_BSS_TABLE; i++) 
	{
		...
		if(Tab->BssEntry[i].VarIEs)
			mem_free(Tab->BssEntry[i].VarIEs);
	#endif
...
#if MEMORY_OPTIMIZATION
		if(pBss->VarIEs)
			mem_free(pBss->VarIEs);
		pBss->VarIEs = (UCHAR*)mem_malloc(LengthVIE + 32);
#endif
	...
	IN PNDIS_802_11_VARIABLE_IEs pVIE)
{
	...
	if (Idx == BSS_NOT_FOUND) 
	{
		if (Tab->BssNr >= MAX_LEN_OF_BSS_TABLE)
	    {
			...
			#if MEMORY_OPTIMIZATION
					if(Tab->BssEntry[Idx].VarIEs)
						mem_free(Tab->BssEntry[Idx].VarIEs);
			#endif
...
VOID MgtMacHeaderInit(
	...
	IN PUCHAR pBssid) 
{
	NdisZeroMemory(pHdr80211, (sizeof(HEADER_802_11)  - 1/*wsum*/));
	...
	IN ULONG Priv) 
{
	...
	{
		#if MLMEQUEUE_ALLOC_INUSE
		if(Queue->Entry[Tail].Msg)
			p_err("Msg != 0");
		Queue->Entry[Tail].Msg = mem_calloc(MsgLen, 1);
		if(!Queue->Entry[Tail].Msg)
		{
			Queue->Entry[Tail].Occupied = FALSE;
			NdisReleaseSpinLock(&(Queue->Lock));
			return FALSE;
		}
		Queue->Entry[Tail].inuse_len = MsgLen;
		#endif
	...
	IN UCHAR OpMode)
{
	...
	if (MlmeQueueFull(Queue, 0)) 
	{
		RTMP_MLME_HANDLER(pAd); //add by lck
		return FALSE;
	}
	...
	IN UCHAR SendId) 
{
	...
	NdisAcquireSpinLock(&(Queue->Lock));
	if (SendId == 0)
		Ans = ((Queue->Num >= /*(MAX_LEN_OF_MLME_QUEUE / 2)edit by lck*/(MAX_LEN_OF_MLME_QUEUE - 2)) || Queue->Entry[Queue->Tail].Occupied);
	else
	...
	if(Ans){
		int i;
		p_err("MlmeQueueFull:%d,%d", Queue->Num,Queue->Entry[Queue->Tail].Occupied );
