...
VOID RtmpRealseCompletion(VOID *Completion)
{
	//p_err_miss;
	INT8U ret;
	wait_event_t wait = (wait_event_t)Completion;
	OSSemDel(wait, OS_DEL_ALWAYS, &ret);
}
...
ULONG RtmpWaitForCompletionTimeout(VOID *Completion, ULONG Expire)
{
	INT ret;
	//p_err_miss;
	 ret = wait_event_timeout((wait_event_t)Completion, Expire);
	 if(ret){
		 p_err("WaitForCompletionTimeout");
	 	return 0; //timeout
	 }else{
	 	return 1;
	 }
...
{
	wake_up((wait_event_t)Completion);
	//p_err_miss;
