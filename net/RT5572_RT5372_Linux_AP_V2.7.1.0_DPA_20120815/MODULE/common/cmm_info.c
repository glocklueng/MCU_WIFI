...
const static struct _PRTMP_PRIVATE_STA_SHOW_CFG_VALUE_PROC{
	PSTRING name;
	...
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
	{
		...
		if (success == TRUE)
		{
				...
				&& (pAd->CommonCfg.bIEEE80211H == TRUE))
			{
				...
				{
					NotifyChSwAnnToPeerAPs(pAd, (PUCHAR)ZERO_MAC_ADDR, pAd->CurrentAddress, 1, pAd->CommonCfg.Channel);
	...
	IN	PSTRING			arg)
{
	...
	/* Get Channel Region (CountryCode)*/
	{
		...
		while (strcmp((PSTRING) ChRegion[loop].CountReg, "") != 0)
		{
			...
			{
				pChRegion = (PCH_REGION)&ChRegion[loop];
				break;
			}
			loop++;
		}
