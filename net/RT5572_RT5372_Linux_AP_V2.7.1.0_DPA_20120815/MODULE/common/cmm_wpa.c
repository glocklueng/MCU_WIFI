    ...
    IN MLME_QUEUE_ELEM  *Elem) 
{
		...
		pEntry->AuthMode == Ndis802_11AuthModeWPA2)
	{
		...
		link_finish((void*)0, (void*)1);	//add by lck
		p_dbg("link finish  3");
...
int pbkdf2_sha1_f(const char *passphrase, const char *ssid,
			 size_t ssid_len, int iterations, unsigned int count,
			 u8 *digest)
{
	F((char*)passphrase, (char*)ssid, ssid_len, iterations, count, digest); 
	return 0;
}
/* 
	...
	IN	UCHAR			apidx)
{
	...
	do
	{
		...
		if (apidx >= MIN_NET_DEVICE_FOR_APCLI)
		{
			...
			if(pAd->ApCfg.ApCliTab[apcliIfidx].RSN_IE == 0)
				pAd->ApCfg.ApCliTab[apcliIfidx].RSN_IE = (UCHAR*)mem_malloc(MAX_LEN_OF_RSNIE);
		#endif
		...
		IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
		{
			...
			#if MEMORY_OPTIMIZATION
			if(pAd->ApCfg.MBSSID[apidx].RSN_IE[0] == 0)
				pAd->ApCfg.MBSSID[apidx].RSN_IE[0] = (UCHAR*)mem_malloc(MAX_LEN_OF_RSNIE);

			if(pAd->ApCfg.MBSSID[apidx].RSN_IE[1] == 0)
				pAd->ApCfg.MBSSID[apidx].RSN_IE[1] = (UCHAR*)mem_malloc(MAX_LEN_OF_RSNIE);
			#endif
	...
	OUT PUCHAR 	pBuf)
{
	...
	NdisZeroMemory(pHdr, sizeof(KDE_HDR) - 1); //edit by lck
	...
	OUT PEAPOL_PACKET   pMsg)
{
	...
	if ((MsgType == EAPOL_PAIR_MSG_3 && bWPA2Capable) || (MsgType == EAPOL_GROUP_MSG_1))
	{
		...
		if (bWPA2Capable)
		{
			...
			data_offset += sizeof(KDE_HDR) - 1; //edit by lck
