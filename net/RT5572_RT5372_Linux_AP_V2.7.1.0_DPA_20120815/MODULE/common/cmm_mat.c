...
extern const MATProtoOps MATProtoIPHandle;
extern const MATProtoOps MATProtoARPHandle;
extern const MATProtoOps MATProtoPPPoEDisHandle;
extern const MATProtoOps MATProtoPPPoESesHandle;
extern const MATProtoOps MATProtoIPv6Handle;

extern const UCHAR SNAP_802_1H[];
extern const UCHAR SNAP_BRIDGE_TUNNEL[];
...
const static MATProtoTable MATProtoTb[]=
{
	...
	IN UCHAR    OpMode)
{
	...
	for (i=0; i < MAX_MAT_SUPPORT_PROTO_NUM; i++)
	{
		...
		{
			pHandle = (struct _MATProtoOps*)MATProtoTb[i].pHandle;	/* the pHandle must not be null! */
	...
	IN RTMP_ADAPTER *pAd)
{
	...
	for (i=0; i<MAX_MAT_SUPPORT_PROTO_NUM; i++)
	{
			pHandle = (struct _MATProtoOps*)MATProtoTb[i].pHandle;
...
/*	pAd->MatCfg.pMATNodeEntryPoll = kmalloc(sizeof(MATNodeEntry) * MAX_MAT_NODE_ENTRY_NUM, GFP_KERNEL); */
	...
	for (i = 0; i < MAX_MAT_SUPPORT_PROTO_NUM; i++)
	{
			pHandle = (MATProtoOps*)MATProtoTb[i].pHandle;
		ASSERT(pHandle);
...
init_failed:
	...
	for (i = 0; i < MAX_MAT_SUPPORT_PROTO_NUM; i++)
	{
		if ((pHandle = (MATProtoOps*)MATProtoTb[i].pHandle) != NULL)
		{
			if (pHandle->exit != NULL)
			{
