...
#define EE_FUSE_ID_NUM  (sizeof(EE_FUSE_ID_LIST) / sizeof(USHORT))

static BOOLEAN  validEepromID(RTMP_ADAPTER *pAd)
{
	USHORT eeFlashId;
	int listIdx;
	
	eFuseRead(pAd, 0, &eeFlashId, 2);
	for(listIdx =0 ; listIdx < EE_FUSE_ID_NUM; listIdx++)
	{
		if (eeFlashId == EE_FUSE_ID_LIST[listIdx])
			return TRUE;
	}
	return FALSE;
}
	...
	OUT USHORT *pValue)
{
	...
	#endif
	{
		if(pAd->chipCap.use_ram_eeprom)
			memcpy(pValue, pAd->chipCap.eebuf + Offset, 2);
		else
...
//如果use_ram_eeprom == 1 表示强制使用ram_eeprom
	if(!validEepromID(pAd)  && pAd->chipCap.eebuf && pAd->chipCap.use_ram_eeprom == 0)
		pAd->chipCap.use_ram_eeprom = 1; //add by lck，如果efuse不可用，就用内存代替
