...
const UCHAR BaSizeArray[4] = {8,16,32,64};
	...
	IN PRTMP_ADAPTER pAd)
{
	...
	)
	{
		for (i = 0; i < Country_Region_GroupNum_2GHZ; i++)
		{
			...
			{
				pChDesc = (PCH_DESC)Country_Region_ChDesc_2GHZ[i].pChDesc;
	...
	)
	{
		...
		for (i = 0; i < Country_Region_GroupNum_5GHZ; i++)
		{
			...
			{
				pChDesc = (PCH_DESC)Country_Region_ChDesc_5GHZ[i].pChDesc;
				num = TotalChNum(pChDesc);
				bRegionFound = TRUE;
				break;
			}
...
CHAR ConvertToRssi(RTMP_ADAPTER *pAd, CHAR Rssi, UCHAR rssi_idx, UCHAR AntSel, UCHAR BW)
{
	...
	signed char ARssiOffset[3];
	signed char BGRssiOffset[3];
	/* Rssi equals to zero or rssi_idx larger than 3 should be an invalid value*/
	if (Rssi == 0 || rssi_idx >= 3)
		return -99;
	ARssiOffset[0] = pAd->ARssiOffset0;
	ARssiOffset[1] = pAd->ARssiOffset1;
	ARssiOffset[2] = pAd->ARssiOffset2;
	
	BGRssiOffset[0] = pAd->BGRssiOffset0;
	BGRssiOffset[1] = pAd->BGRssiOffset1;
	BGRssiOffset[2] = pAd->BGRssiOffset2;
	
	LNAGain = GET_LNA_GAIN(pAd);
	if (pAd->LatchRfRegs.Channel > 14)
		RssiOffset = ARssiOffset[rssi_idx];
	else
		RssiOffset = BGRssiOffset[rssi_idx];
	...
	IN UCHAR OpMode) 
{
	...
	if ((pAd->MlmeAux.Channel == 0) || ScanPending) 
	{
		...
		if (pAd->CommonCfg.BBPCurrentBW == BW_40)
		{
			...
			RTMPSendWirelessEvent(pAd, IW_SCAN_COMPLETED_EVENT_FLAG, NULL, BSS0, 0); //ADD by lck
	...
	else 
	{
			...
			)
		{
...

			{
				...
				if (OpMode == OPMODE_AP)
				{
					MgtMacHeaderInitExt(pAd, &Hdr80211, SUBTYPE_PROBE_REQ, 0, (PUCHAR)BROADCAST_ADDR, 
										pAd->ApCfg.MBSSID[0].Bssid,
										(PUCHAR)BROADCAST_ADDR);
				}
#endif /* CONFIG_AP_SUPPORT */

				MakeOutgoingFrame(pOutBuffer,               &FrameLen,
								  (sizeof(HEADER_802_11)  - 1/*wsum*/),    &Hdr80211,
								  1,                        &SsidIe,
								  1,                        &SsidLen,
