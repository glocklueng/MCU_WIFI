...
const UCHAR    BIT8[] = {0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80};
const UCHAR    NUM_BIT8[] = {0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80};
...
#ifdef  MT7601
extern const REG_PAIR   BBPRegTable[];
#else
...
const RTMP_REG_PAIR	APMACRegTable[] =	{
...
/*		pBeaconBuf = kmalloc(MAX_BEACON_SIZE, MEM_ALLOC_FLAG);*/
		...
		index = sizeof(RTMP_ADAPTER);
		#if 0
		p_dbg("sizeof(RTMP_ADAPTER) = %d\n", index);
		p_dbg("sizeof(RTMP_CHIP_OP) = %d\n", sizeof(RTMP_CHIP_OP));
		//p_dbg("sizeof(RTMP_CHIP_CAP) = %d\n", sizeof(RTMP_CHIP_CAP));
		p_dbg("sizeof(CmdQP) = %d\n", sizeof(CmdQ));
		p_dbg("sizeof(RTMP_TIMER_TASK_QUEUE) = %d\n", sizeof(RTMP_TIMER_TASK_QUEUE));
		p_dbg("sizeof(HT_TX_CONTEXT) = %d\n", sizeof(HT_TX_CONTEXT));
		p_dbg("sizeof(QUEUE_HEADER) = %d\n", sizeof(QUEUE_HEADER));
		p_dbg("sizeof(RTMP_DMABUF) = %d\n", sizeof(RTMP_DMABUF));
		p_dbg("sizeof(RTMP_MGMT_RING) = %d\n", sizeof(RTMP_MGMT_RING));
		p_dbg("sizeof(RX_CONTEXT) = %d\n", sizeof(RX_CONTEXT));
		p_dbg("sizeof(RTMP_RF_REGS) = %d\n", sizeof(RTMP_RF_REGS));
		p_dbg("sizeof(EEPROM_ANTENNA_STRUC) = %d\n", sizeof(EEPROM_ANTENNA_STRUC));
		p_dbg("sizeof(EEPROM_NIC_CONFIG2_STRUC) = %d\n", sizeof(EEPROM_NIC_CONFIG2_STRUC));
		p_dbg("sizeof(SOFT_RX_ANT_DIVERSITY) = %d\n", sizeof(SOFT_RX_ANT_DIVERSITY));
		p_dbg("sizeof(CHANNEL_TX_POWER) = %d\n", sizeof(CHANNEL_TX_POWER)*MAX_NUM_OF_CHANNELS);
		p_dbg("sizeof(CHANNEL_TX_POWER) = %d\n", sizeof(CHANNEL_TX_POWER)*MAX_NUM_OF_CHANNELS);
		p_dbg("sizeof(TX_POWER_CONTROL) = %d\n", sizeof(TX_POWER_CONTROL));
		p_dbg("sizeof(TXWI_STRUC) = %d\n", sizeof(TXWI_STRUC));
		p_dbg("sizeof(PSPOLL_FRAME) = %d\n", sizeof(PSPOLL_FRAME));
		p_dbg("(sizeof(HEADER_802_11)  - 1/*wsum*/) = %d\n", (sizeof(HEADER_802_11)  - 1/*wsum*/));
		p_dbg("sizeof(TX_CONTEXT) = %d\n", sizeof(TX_CONTEXT)*2);
		p_dbg("sizeof(STA_ADMIN_CONFIG) = %d\n", sizeof(STA_ADMIN_CONFIG));
		p_dbg("sizeof(STA_ACTIVE_CONFIG) = %d\n", sizeof(STA_ACTIVE_CONFIG));
		p_dbg("sizeof(NDIS_MEDIA_STATE) = %d\n", sizeof(NDIS_MEDIA_STATE));
		p_dbg("sizeof(NDIS_MEDIA_STATE) = %d\n", sizeof(NDIS_MEDIA_STATE));
		p_dbg("sizeof(COMMON_CONFIG) = %d\n", sizeof(COMMON_CONFIG));
		p_dbg("sizeof(MLME_STRUCT) = %d\n", sizeof(MLME_STRUCT));
		p_dbg("sizeof(MLME_AUX) = %d\n", sizeof(MLME_AUX));
		p_dbg("sizeof(MLME_STRUCT) = %d\n", sizeof(MLME_STRUCT));
		p_dbg("sizeof(BSS_TABLE) = %d\n", sizeof(BSS_TABLE));
		p_dbg("sizeof(MAC_TABLE) = %d\n", sizeof(MAC_TABLE));
		p_dbg("sizeof(CIPHER_KEY) = %d\n", sizeof(CIPHER_KEY));
		p_dbg("sizeof(FRAGMENT_FRAME) = %d\n", sizeof(FRAGMENT_FRAME));
		p_dbg("sizeof(COUNTER_802_3) = %d\n", sizeof(COUNTER_802_3));
		p_dbg("sizeof(COUNTER_802_11) = %d\n", sizeof(COUNTER_802_11));
		p_dbg("sizeof(COUNTER_RALINK) = %d\n", sizeof(COUNTER_RALINK));
		p_dbg("sizeof(COUNTER_DRS) = %d\n", sizeof(COUNTER_DRS));
		p_dbg("sizeof(PRIVATE_STRUC) = %d\n", sizeof(PRIVATE_STRUC));
		p_dbg("sizeof(EXT_CAP_INFO_ELEMENT) = %d\n", sizeof(EXT_CAP_INFO_ELEMENT));
		p_dbg("sizeof(LIST_HEADER) = %d\n", sizeof(LIST_HEADER));
		#endif
//		Status = AdapterBlockAllocateMemory(handle, (PVOID *)&pAd, index);
		pAd = &gAd; 
...

{
	...
	for(j = 0; j < 4; j++)
	{
		for(i = 0; i < 8; i++)
		{
			RT28xx_EEPROM_READ16(pAd, j*16 + i*2, value);
			memcpy(&test[i*2], &value, 2);
		}
		dump_hex("", test, 16);
	}
}
	...
	pAd->PermanentAddress[5] &= 0xfe; //add by lck
	...
	{//如果使用ram上的虚拟eeprom，就需要使用 自动生成的mac地址
		if(pAd->chipCap.use_ram_eeprom){
			create_mac(pAd->CurrentAddress);
		}else
...
VOID AsicInitBcnBuf(IN RTMP_ADAPTER *pAd)
{
	...
	assert(pChipCap->BcnMaxHwNum <= HW_BEACON_MAX_NUM);
	...
	IN  BOOLEAN		bHardReset)
{
	...
	p_MACRegTable = (RTMP_REG_PAIR	*)mem_malloc(512);//512足够
	
	if(!p_MACRegTable)
			return NDIS_STATUS_FAILURE;
		memcpy(p_MACRegTable, MACRegTable, 512);	//
...
//	UsbCfg.word = 0;
	...
	{
#ifdef RT30xx
		if ((p_MACRegTable[Index].Register == TX_SW_CFG0) &&
			(IS_RT3070(pAd) || IS_RT3071(pAd) || IS_RT3390(pAd)))
		{
			p_MACRegTable[Index].Value = 0x00000400;
		}
#endif /* RT30xx */
		RTMP_IO_WRITE32(pAd, (USHORT)p_MACRegTable[Index].Register, p_MACRegTable[Index].Value);
	}
#endif /* RTMP_MAC_USB */
...
#ifdef RLT_MAC
	{
	UINT32 *MACValue = (UINT32*)mem_malloc(128 * 2*sizeof(UINT32));
...
#else
{
	...
	if (bHardReset)
	{
		...
		{
			UINT32 *MACValue = (UINT32*)mem_malloc(256*sizeof(UINT32));
...
========================================================================
*/
static LIST_RESOURCE_OBJ_ENTRY g_timer_entry[12];
static uint8_t g_timer_entry_cnt = 0;
	...
	IN	VOID					*pRsc)
{
	...
	if(g_timer_entry_cnt < 12)		//edit by lck
		pObj = &g_timer_entry[g_timer_entry_cnt++];
	else
...
/*
	...
	========================================================================
*/
DECLARE_MONITOR_ITEM("rtmp timer", rtmp_timer);
VOID	RTMPInitTimer(
	IN	PRTMP_ADAPTER			pAd,
	...
	IN	BOOLEAN					Repeat)
{
	...
	INC_MONITOR_ITEM_VALUE(rtmp_timer);
	...
	pTimer->pAd = pAd;
        ...
        if(pTimer->Valid && pTimer->TimerObj) 
            p_dbg("timer is valid");
        else
	...
	OUT	BOOLEAN					*pCancelled)
{
	...
	{
		p_err("RTMPCancelTimer failed, Timer hasn't been initialize!\n");
	}
...
{
	RTMP_SEM_LOCK(&TimerSemLock);
	DEC_MONITOR_ITEM_VALUE(rtmp_timer);
	if (pTimer->Valid)
	{
		if (pTimer->State == FALSE)
			pTimer->Repeat = FALSE;
	...
	{
		p_err_fun;
		DBGPRINT(RT_DEBUG_ERROR,("RTMPReleasefailed, Timer hasn't been initialize!\n"));
	}
