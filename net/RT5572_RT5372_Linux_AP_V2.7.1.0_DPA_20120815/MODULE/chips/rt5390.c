...
0x8e, 0x75, 0x01, 0x43, 0x11, 0x08, 0x27, 0x00, 0xff, 0xff, 0x16, 0x01, 0xff, 0xff, 0xd9, 0xfa, 
	...
	IN PRTMP_ADAPTER		pAd)
{
	...
	/* Init capability */
	if (IS_RT5390(pAd))
		pChipCap->pRFRegTable = (REG_PAIR*)RF5390RegTable;
	else if (IS_RT5392(pAd))
		pChipCap->pRFRegTable = (REG_PAIR*)RF5392RegTable;
...
//#ifdef RTMP_FLASH_SUPPORT
	pChipCap->use_ram_eeprom = FALSE;
	pChipCap->eebuf = (UCHAR*)mem_malloc(EEPROM_SIZE);
	memcpy(pChipCap->eebuf,	RT5390_EeBuffer, EEPROM_SIZE);
//#endif /* RTMP_FLASH_SUPPORT */
	...
	IN PRTMP_ADAPTER 		pAd)
{
	...
	if (IS_RT5392(pAd))
	{
		p_RFRegTable = (REG_PAIR *)mem_malloc(sizeof(RF5392RegTable));	
		if(!p_RFRegTable)
		return;

		memcpy(p_RFRegTable, RF5392RegTable, sizeof(RF5392RegTable));
		/* Initialize RF register to default value */
		for (i = 0; i < NUM_RF_5392_REG_PARMS; i++)
		{
			if (IS_RT5392C(pAd))
			{
				...
				if (p_RFRegTable[i].Register == RF_R23)
				{
					p_RFRegTable[i].Value = 0x0F;
				}
				else if (p_RFRegTable[i].Register == RF_R24)
				{
					p_RFRegTable[i].Value = 0x3E;
				}
				else if (p_RFRegTable[i].Register == RF_R51)
				{
					p_RFRegTable[i].Value = 0x32;
				}
				else if (p_RFRegTable[i].Register == RF_R53)
				{
					p_RFRegTable[i].Value = 0x22;
				}
				else if (p_RFRegTable[i].Register == RF_R56)
				{
					p_RFRegTable[i].Value = 0xC1;
				}
				else if (p_RFRegTable[i].Register == RF_R59)
				{
					p_RFRegTable[i].Value = 0x0F;
				}
#endif /* RTMP_MAC_USB */
			}	
			RT30xxWriteRFRegister(pAd, p_RFRegTable[i].Register, p_RFRegTable[i].Value);
		}
		mem_free(p_RFRegTable);
	}
	...
	{
		p_RFRegTable = (REG_PAIR *)mem_malloc(sizeof(RF5390RegTable));	
		if(!p_RFRegTable)
		return;

		memcpy(p_RFRegTable, RF5390RegTable, sizeof(RF5390RegTable));
		/* Initialize RF register to default value */
		for (i = 0; i < NUM_RF_5390_REG_PARMS; i++)
		{
#ifdef RTMP_MAC_USB /* For >= RT5370F */
			if (IS_RT5390F(pAd) && (p_RFRegTable[i].Register == RF_R06))
			{
				p_RFRegTable[i].Value = 0xE0;
			}
			else if (IS_RT5390F(pAd) && (p_RFRegTable[i].Register == RF_R25))
			{
				p_RFRegTable[i].Value = 0x80;
			}
			else if (IS_RT5390F(pAd) && (p_RFRegTable[i].Register == RF_R46))
			{
				p_RFRegTable[i].Value = 0x73;
			}
			else if (IS_RT5390F(pAd) && (p_RFRegTable[i].Register == RF_R53))
			{
				p_RFRegTable[i].Value = 0x00;
			}
			else if (IS_RT5390F(pAd) && (p_RFRegTable[i].Register == RF_R56))
			{
				p_RFRegTable[i].Value = 0x42;
			}
			else if (IS_RT5390F(pAd) && (p_RFRegTable[i].Register == RF_R61))
			{
				p_RFRegTable[i].Value = 0xD1;
			}
#endif /* RTMP_MAC_USB */
			RT30xxWriteRFRegister(pAd, p_RFRegTable[i].Register, p_RFRegTable[i].Value);
		}
	...
	IN PRTMP_ADAPTER		pAd)
{
	...
	RT5390_desiredTSSIOverCCKExt = (CHAR (*)[NUM_OF_CH_FOR_PER_CH_TX_PWR_OFFSET + 1][4])mem_malloc((NUM_OF_CH_FOR_PER_CH_TX_PWR_OFFSET + 1)*4);

	RT5390_desiredTSSIOverOFDMExt = (CHAR (*)[NUM_OF_CH_FOR_PER_CH_TX_PWR_OFFSET + 1][8])mem_malloc((NUM_OF_CH_FOR_PER_CH_TX_PWR_OFFSET + 1)*8);

	RT5390_desiredTSSIOverHTExt = (CHAR (*)[NUM_OF_CH_FOR_PER_CH_TX_PWR_OFFSET + 1][8])mem_malloc((NUM_OF_CH_FOR_PER_CH_TX_PWR_OFFSET + 1)*8);

	RT5390_desiredTSSIOverHT40Ext = (CHAR (*)[NUM_OF_CH_FOR_PER_CH_TX_PWR_OFFSET + 1][8])mem_malloc((NUM_OF_CH_FOR_PER_CH_TX_PWR_OFFSET + 1)*8);

	RT5390_desiredTSSIOverHTUsingSTBCExt = (CHAR (*)[NUM_OF_CH_FOR_PER_CH_TX_PWR_OFFSET + 1][8])mem_malloc((NUM_OF_CH_FOR_PER_CH_TX_PWR_OFFSET + 1)*8);
	...
	if ((pAd->TxPowerCtrl.bInternalTxALC == TRUE) && (IS_RT5390(pAd)))
	{
		if ((pAd->Mlme.OneSecPeriodicRound % 4 == 0) && (*pDeltaPowerByBbpR1 == 0))
		{
			...
			else
			{
				...
				pTxPowerTuningEntry = (PTX_POWER_TUNING_ENTRY_STRUCT)&TxPowerTuningTable[TuningTableIndex + TX_POWER_TUNING_ENTRY_OFFSET ];
...
#else
	BBP_RESET_CTL 	*BBPResetCtl = (BBP_RESET_CTL*)mem_malloc(sizeof(BBP_RESET_CTL));
	DBGPRINT(RT_DEBUG_ERROR, ("Reset BBP Agent busy bit!!\n"));
	
	if(!BBPResetCtl)
		return;
	NICStoreBBPValue(pAd, BBPResetCtl->BBPRegDB);
	RTMP_IO_WRITE32(pAd, MAC_SYS_CTRL, 0xe);
	RTMPusecDelay(50);
	RTMP_IO_WRITE32(pAd, MAC_SYS_CTRL, 0xc);
	RTMPusecDelay(50);
	NICRestoreBBPValue(pAd, BBPResetCtl->BBPRegDB);
	mem_free(BBPResetCtl);
#endif
}
