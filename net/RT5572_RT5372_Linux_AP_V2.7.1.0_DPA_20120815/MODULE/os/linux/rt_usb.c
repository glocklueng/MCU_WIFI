...
========================================================================
 */
#define RTMP_TIMER_STK_SIZE		RTMP_TIMER_STACK_SIZE
#define MLME_STK_SIZE			MLME_STACK_SIZE
#define RT_USBCMD_STK_SIZE		RT_USBCMD_STACK_SIZE

static OS_STK RtmpTimerQThreadSTK[RTMP_TIMER_STK_SIZE];
static OS_STK MlmeThreadSTK[MLME_STK_SIZE];
static OS_STK RTUSBCmdThreadSTK[RT_USBCMD_STK_SIZE];
...
NDIS_STATUS RtmpMgmtTaskInit(IN RTMP_ADAPTER *pAd)
{
	...
	RTMP_OS_TASK_INIT(pTask, "RtmpTimerTask", pAd);
	status = RtmpOSTaskAttach(pTask, RtmpTimerQThread, (ULONG)pTask,
	RtmpTimerQThreadSTK, RTMP_TIMER_STK_SIZE, RTMP_TIMER_PRIO);
	if (status == NDIS_STATUS_FAILURE)
	{
		printk("%s: unable to start RtmpTimerQThread\n", RTMP_OS_NETDEV_GET_DEVNAME
	(pAd->net_dev));
		return NDIS_STATUS_FAILURE;
	}
	...
	pTask = &pAd->mlmeTask;
	RTMP_OS_TASK_INIT(pTask, "RtmpMlmeTask", pAd);
	status = RtmpOSTaskAttach(pTask, MlmeThread, (ULONG)pTask, MlmeThreadSTK,
	MLME_STK_SIZE, MLME_PRIO);
	if (status == NDIS_STATUS_FAILURE)
	{
	...
	RTMP_OS_TASK_INIT(pTask, "RtmpCmdQTask", pAd);
	status = RtmpOSTaskAttach(pTask, RTUSBCmdThread, (ULONG)pTask,
	RTUSBCmdThreadSTK, RT_USBCMD_STK_SIZE, RT_USBCMD_PRIO);
	if (status == NDIS_STATUS_FAILURE)
	{
		printk("%s: unable to start RTUSBCmdThread\n", RTMP_OS_NETDEV_GET_DEVNAME(pAd
	->net_dev));
		return NDIS_STATUS_FAILURE;
	}
	...
	pAd->timerTask.priv = (void*)pAd;
	err = OSTaskCreateExt(RtmpTimerQThread, (void*) &pAd->timerTask, (OS_STK*)
	&RtmpTimerQThreadSTK[RTMP_TIMER_STK_SIZE - 1], (INT8U)RTMP_TIMER_PRIO, 
	(INT16U)RTMP_TIMER_PRIO, (OS_STK*) &RtmpTimerQThreadSTK[0], (INT32U)
	RTMP_TIMER_STK_SIZE, (void*)0, (INT16U)(OS_TASK_OPT_STK_CHK |
	OS_TASK_OPT_STK_CLR));
	if (err != OS_ERR_NONE)
	{
		goto end;
	}
	strcpy(pAd->timerTask.taskName, "RTMP_TIMER");
	RTMP_SEM_EVENT_INIT_LOCKED(&(pTask->taskSema), pSemList);
	pTask->taskPID = THREAD_PID_INIT_VALUE;

	OSTaskNameSet(RTMP_TIMER_PRIO, (INT8U*)"RTMP_TIMER", (INT8U*) &err);
	if (err != OS_ERR_NONE)
	{
		goto end;
	}

	pAd->mlmeTask.priv = (void*)pAd;
	err = OSTaskCreateExt(MlmeThread, (void*) &pAd->mlmeTask, (OS_STK*)
	&MlmeThreadSTK[MLME_STK_SIZE - 1], (INT8U)MLME_PRIO, (INT16U)MLME_PRIO, 
	(OS_STK*) &MlmeThreadSTK[0], (INT32U)MLME_STK_SIZE, (void*)0, (INT16U)
	(OS_TASK_OPT_STK_CHK | OS_TASK_OPT_STK_CLR));
	if (err != OS_ERR_NONE)
	{
		goto end;
	}
	OSTaskNameSet(MLME_PRIO, (INT8U*)"MLME", (INT8U*) &err);
	if (err != OS_ERR_NONE)
	{
		goto end;
	}

	pAd->cmdQTask.priv = (void*)pAd;
	err = OSTaskCreateExt(RTUSBCmdThread, (void*) &pAd->cmdQTask, (OS_STK*)
	&RTUSBCmdThreadSTK[RT_USBCMD_STK_SIZE - 1], (INT8U)RT_USBCMD_PRIO, (INT16U)
	RT_USBCMD_PRIO, (OS_STK*) &RTUSBCmdThreadSTK[0], (INT32U)RT_USBCMD_STK_SIZE,
	(void*)0, (INT16U)(OS_TASK_OPT_STK_CHK | OS_TASK_OPT_STK_CLR));
	if (err != OS_ERR_NONE)
	{
		goto end;
	}
	OSTaskNameSet(RT_USBCMD_PRIO, (INT8U*)"RT_USBCMD", (INT8U*) &err);
	if (err != OS_ERR_NONE)
	{
		goto end;
	}
	#endif 
	end: return NDIS_STATUS_SUCCESS;
}
...
static void rtusb_null_frame_done_tasklet(unsigned long data)
{
	...
	p_dbg_enter;
	pUrb = (purbb_t)data;
	pNullContext = (PTX_NULL_CONTEXT)pUrb->context;
...
{
	PRTMP_ADAPTER pAd;
	PTX_NULL_CONTEXT pPsPollContext;
	purbb_t pUrb;
	NTSTATUS Status;
...
/*	pPsPollContext	= (PTX_CONTEXT)pUrb->context; */
	pPsPollContext	= (PTX_NULL_CONTEXT)RTMP_USB_URB_DATA_GET(pUrb);
	Status			= RTMP_USB_URB_STATUS_GET(pUrb);
...
const uint8_t rx_timeout_tb[20] = 
{
	0, 0, 0, 0, 0, 0, 10, 10, 10, 20, 20, 20, 30, 30, 30, 30, 30, 50, 50,
	50, 
};
...
/*	Status = pUrb->status; */
	...
	if (Status == USB_ST_NOERROR)
	{
		...
		{
			if (rx_timeout_cnt >= 20)
				rx_timeout_cnt = 0;
			if (rx_timeout_tb[rx_timeout_cnt])
				mdelay(rx_timeout_tb[rx_timeout_cnt]);
			rx_timeout_cnt++;
		}
	}
	else	 /* STATUS_OTHER */
...
/*	Status			= pUrb->status; */
	...
	//mutex_unlock(pAd->mlme_transmit_lock);
...
static void os_tasklet_thread(RTMP_ADAPTER *pAd)
{
	int ret;
	while (!pAd->tasklet_thread_event)
		mdelay(1000);
	while (1)
	{
		ret = wait_event_timeout(pAd->tasklet_thread_event, 5000);
		if (ret != 0)
		{
			if (ret != WAIT_EVENT_TIMEOUT)
			{
				p_err("os_tasklet_thread wait err:%d, %x\n", ret, pAd->tasklet_thread_event);
				mdelay(2000);
			}
			continue;
		}

		if (pAd->RscTaskletMemList.size > 0)
		{
			while (pAd->RscTaskletMemList.size)
			{
				//				OS_SEM_LOCK(&UtilSemLock);
				PLIST_ENTRY entry = removeHeadList(&pAd->RscTaskletMemList);
...
#define OS_TASKLET_STK_SIZE 	OS_TASKLET_STACK_SIZE
static OS_STK os_tasklet_stk[OS_TASKLET_STK_SIZE];
	...
	IN RTMP_ADAPTER *pAd)
{
	...
	pAd->tasklet_thread_event = 0;

	 thread_create(os_tasklet_thread,
				pAd,
				OS_TASKLET_PRIO,
				os_tasklet_stk,
				OS_TASKLET_STK_SIZE,
	...
	pAd->tasklet_thread_event = init_event();
