...
static unsigned char *skb_tail_pointer(const struct sk_buff *skb)
{
...
static void skb_set_tail_pointer(struct sk_buff *skb, const int offset)
{
...
static void skb_copy_from_linear_data(const struct sk_buff *skb, void *to,
	const unsigned int len)
{
	memcpy(to, skb->data, len);
} 

static void skb_copy_from_linear_data_offset(const struct sk_buff *skb, const
...
/*
static void nf_copy(struct sk_buff *dst, const struct sk_buff *src)
{
	#if defined(CONFIG_NF_CONNTRACK) || defined(CONFIG_NF_CONNTRACK_MODULE)
...
{
	printk("skb_over_panic: text:%p len:%d put:%d head:%p "
...
{
	printk("skb_under_panic: text:%p len:%d put:%d head:%p "
	"data:%p tail:%#lx end:%#lx dev:%s\n", here, skb->len, sz, skb->head, skb
	->data, (unsigned long)skb->tail, (unsigned long)skb->end, skb->dev ? skb
	->dev->name: "<NULL>");
	BUG();
	...
	node)
{
	...
	nodata: kmem_cache_free(cache, skb);
	skb = NULL;
	goto out;
}
...
{
	//if(skb->truesize == 156 + sizeof(struct sk_buff))
	//	p_dbg("free:%x, %x", skb, skb->head);
	//else
	//	p_dbg("free1:%d, %x, %x", skb->truesize - sizeof(struct sk_buff), skb, skb->head);
	skb_release_all(skb);
	kfree_skbmem(skb);
