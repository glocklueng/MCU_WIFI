...
char const * const pWirelessSysEventText[IW_SYS_EVENT_TYPE_NUM] = {    
	/* system status event */
...
char const * const pWirelessSpoofEventText[IW_SPOOF_EVENT_TYPE_NUM] = {   	
...
char const * const pWirelessFloodEventText[IW_FLOOD_EVENT_TYPE_NUM] = {   	
...
char const * const pWirelessWscEventText[IW_WSC_EVENT_TYPE_NUM] = {   	
	...
	IN	PRTMP_ADAPTER pAd)
{
	...
	pAd->CommonCfg.CountryRegion = REGION_5_BG_BAND;
	strcpy((char*)pAd->CommonCfg.CountryCode, "CN");

	pAd->ApCfg.BssidNum = 1;
	
	strcpy((char*)pAd->ApCfg.MBSSID[0].Ssid, "wsum_ap");
	pAd->ApCfg.MBSSID[0].SsidLen = strlen("wsum_ap");

	pAd->ApCfg.MBSSID[0].MaxStaNum = MAX_STA_NUM;

	pAd->CommonCfg.BasicRateBitmap = 15;
	...
	pAd->ApCfg.BssEdcaParm.Cwmin[2] = 32;
	...
	pAd->ApCfg.BssEdcaParm.Txop[2] = 94;
	pAd->ApCfg.BssEdcaParm.Txop[3] = 47;

	pAd->CommonCfg.bUseShortSlotTime = TRUE;
	pAd->ApCfg.MBSSID[0].IsolateInterStaTraffic = FALSE;
	pAd->ApCfg.IsolateInterStaTrafficBTNBSSID = FALSE;
//	pAd->CommonCfg.RadarDetect.CSPeriod = 10;
	/*
	pAd->ApCfg.AuthFloodThreshold = 32;
	pAd->ApCfg.AssocReqFloodThreshold = 32;
	pAd->ApCfg.ReassocReqFloodThreshold = 32;
	pAd->ApCfg.ProbeReqFloodThreshold = 32;
	pAd->ApCfg.DisassocFloodThreshold = 32;
	pAd->ApCfg.DeauthFloodThreshold = 32;
	pAd->ApCfg.EapReqFooldThreshold = 32;*/

	pAd->ApCfg.MBSSID[0].DefaultKeyId = 1;

	pAd->ApCfg.MBSSID[0].DesiredTransmitSetting.field.MCS = MCS_AUTO;
	//pAd->ApCfg.MBSSID[0].radius_srv_info[srv_idx].radius_ip
	//pAd->ApCfg.own_ip_addr

#ifdef  DOT11_N_SUPPORT
	pAd->CommonCfg.PhyMode = PHY_11BGN_MIXED;
	pAd->CommonCfg.DesiredPhyMode = PHY_11BGN_MIXED;
#else
	pAd->CommonCfg.PhyMode = PHY_11BG_MIXED;
	pAd->CommonCfg.DesiredPhyMode = PHY_11BG_MIXED;
#endif
	/*pAd->StaCfg.StaOriGeography = */pAd->CommonCfg.Geography = 1;
	//pAd->StaCfg.BssType = BSS_INFRA;
	pAd->CommonCfg.BeaconPeriod = 100;
	pAd->CommonCfg.TxPowerDefault = 100;
	pAd->CommonCfg.TxPowerPercentage = 100;
	pAd->CommonCfg.RtsThreshold = 2347;
	pAd->CommonCfg.FragmentThreshold = 2346;
	pAd->CommonCfg.bUseZeroToDisableFragment = FALSE;
	pAd->CommonCfg.bEnableTxBurst = TRUE; //mark 突发  TX PACKET BURST
	pAd->CommonCfg.bWmmCapable = FALSE;
	pAd->CommonCfg.bAggregationCapable = FALSE;
	pAd->ApCfg.MBSSID[0].bWmmCapableOrg = \
		pAd->ApCfg.MBSSID[0].bWmmCapable = FALSE;
	...
	{
		pAd->ApCfg.ApCliTab[i].UapsdInfo.bAPSDCapable = 1;
	}
	pAd->ApCfg.MBSSID[0].UapsdInfo.bAPSDCapable = 1;
	pAd->ApCfg.MBSSID[0].bDLSCapable = TRUE;

				
	pAd->CommonCfg.AckPolicy[0] = 0;
	pAd->CommonCfg.AckPolicy[1] = 0;
	pAd->CommonCfg.AckPolicy[2] = 0;
	pAd->CommonCfg.AckPolicy[3] = 0;
	//RTMP_SET_PSM_BIT(pAd, PWR_ACTIVE);
	OPSTATUS_SET_FLAG(pAd, fOP_STATUS_RECEIVE_DTIM);

	pAd->CommonCfg.bWirelessEvent = TRUE;  //开启事件通知
	...
	pAd->CommonCfg.BACapability.field.MpduDensity = 5;//4//
	...
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
	{
		...
		{
			Value = /*simple_strtol(Bufptr, 0, 10)*/33;			
//			if ((Value >= 0 && Value <= 15) || (Value == 32))
			...
			pAd->ApCfg.MBSSID[i].PhyMode = PHY_11BGN_MIXED;
