...
/* timeout -- ms */
VOID RTMP_SetPeriodicTimer(IN NDIS_MINIPORT_TIMER *pTimer, IN unsigned long
	timeout)
{
...
{
	OS_NDIS_MINIPORT_TIMER *pTimer_tmp;
	PRALINK_TIMER_STRUCT timer_struct = (PRALINK_TIMER_STRUCT)data;

	p_dbg("timer_struct:%x", timer_struct);

	pTimer_tmp =  timer_setup(timer_struct->TimerValue, timer_struct->PeriodicType, (void*)(function), (void *)data);

	*pTimerOrg = (void*)pTimer_tmp;

}


VOID RTMP_OS_Add_Timer(IN NDIS_MINIPORT_TIMER *pTimer, IN unsigned long timeout)
{
...
VOID RTMP_OS_Mod_Timer(IN NDIS_MINIPORT_TIMER *pTimer, IN unsigned long timeout)
{
	mod_timer(*pTimer, timeout);
}
...
//í￡?1?¨ê±?÷
VOID RTMP_OS_Del_Timer(IN NDIS_MINIPORT_TIMER *pTimer, OUT BOOLEAN *pCancelled)
{

	if(del_timer(*pTimer) == OS_ERR_NONE)
		*pCancelled = TRUE;
	else
		*pCancelled = FALSE;
}

VOID RTMP_OS_Free_Timer(IN NDIS_MINIPORT_TIMER *pTimer)
{
	timer_free(*pTimer);
}
...
#if USE_MEM_DEBUG
NDIS_STATUS os_alloc_mem_ex(
	char *name,
	IN VOID *pReserved,
	OUT UCHAR **mem,
	IN ULONG size)
{
	*mem = (PUCHAR) mem_malloc_ex(name, size);
	if (*mem) {
#ifdef VENDOR_FEATURE4_SUPPORT
		OS_NumOfMemAlloc++;
#endif /* VENDOR_FEATURE4_SUPPORT */
...
static void __RtmpOSFSInfoChange(OS_FS_INFO *pOSFSInfo, BOOLEAN bSet)
{
	p_dbg_enter;
	#if 0
 ...
 *******************************************************************************/
static NDIS_STATUS __RtmpOSTaskKill(IN OS_TASK *pTask)
{
	/*	RTMP_ADAPTER *pAd; */
	...
	->taskName, GET_PID_NUMBER(pTask->taskPID)));
		mb();
		pTask->task_killed = 1;
		mb();
	...
	->taskName, GET_PID_NUMBER(pTask->taskPID), ret);
		}
		else
		{
...
NDIS_STATUS __RtmpOSTaskAttach(IN OS_TASK *pTask, IN RTMP_OS_TASK_CALLBACK fn,
	IN ULONG arg, IN void *stk, IN int stk_size, IN int pri)
{
	...
	#else 
	//	pid_number =
	//	        fddfkernel_thread((cast_fn) fn, (void *)arg, RTMP_OS_MGMT_TASK_FLAGS);
	
	pid_number = thread_create(fn,
				(void*)arg,
				pri,
				stk,
				stk_size,
				pTask->taskName[0] ? pTask->taskName: "unkown");
...
static NDIS_STATUS __RtmpOSTaskInit(IN OS_TASK *pTask, IN PSTRING pTaskName, IN
	VOID *pPriv, IN LIST_HEADER *pSemList)
{
	int len;
	p_dbg_enter;
	#if 1
	...
	#ifdef KTHREAD_SUPPORT
	pTask->kthread_q = init_waitqueue_head();
	#endif /* KTHREAD_SUPPORT */
	#endif 
	return NDIS_STATUS_SUCCESS;
}
...
BOOLEAN __RtmpOSTaskWait(IN VOID *pReserved, IN OS_TASK *pTask, IN INT32
	*pStatus)
{
	INT32 stat;
	#if 0
	...
	#else 
	#if 1
	RTMP_SEM_EVENT_WAIT(&(pTask->taskSema), stat);
...
NDIS_STATUS RtmpOSTaskAttach(IN OS_TASK *pTask, IN RTMP_OS_TASK_CALLBACK
	fn, IN ULONG arg, IN void *stk, IN int stk_size, IN int pri)
{
	return __RtmpOSTaskAttach(pTask, fn, arg, stk, stk_size, pri);
}

NDIS_STATUS RtmpOSTaskInit(IN OS_TASK *pTask, IN PSTRING pTaskName, IN
	VOID *pPriv, IN LIST_HEADER *pTaskList, IN LIST_HEADER *pSemList)
{
	return __RtmpOSTaskInit(pTask, pTaskName, pPriv, pSemList);
}

BOOLEAN RtmpOSTaskWait(IN VOID *pReserved, IN OS_TASK *pTask, IN INT32
	*pStatus)
{
...
_wireless_event_callback wireless_event_callback = 0;
			    ...
			    IN UINT32 dataLen) 
{
	...
	if(wireless_event_callback && (flags >= IW_SYS_EVENT_FLAG_START))
		wireless_event_callback(flags);
	...
	*new_dev_p = NULL;
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("Allocate a net device with private data size=%d!\n",
		  privDataSize));
#if LINUX_VERSION_CODE <= 0	/* Red Hat 7.1 */
	*new_dev_p = alloc_netdev(privDataSize, "eth%d", ether_setup);
#else
	//*new_dev_p = alloc_etherdev(privDataSize);
	*new_dev_p = (PNET_DEV)kcalloc(sizeof(struct net_device), 1, 0);
	//strcpy(new_dev_p->name , "eth0");
#endif /* LINUX_VERSION_CODE */
			    ...
			    IN PSTRING pNamePrefix)
{
	...
	pNetDev->flags |= IFF_UP | IFF_BROADCAST | IFF_MULTICAST; //add by lck
...
VOID RtmpOsSetNetDevPriv(IN VOID *pDev,
	...
	pDevInfo->priv_flags = priv_flags;
...
BOOLEAN RtmpOsTaskletSche(IN RTMP_NET_TASK_STRUCT *pTasklet)
{
	...
	work = (struct work_struct*)pTasklet->pContent;

	//	OS_SEM_LOCK(&UtilSemLock);	EDIT BY LCK
	insertTailList(work->pTaskletList, &work->entry);
...
========================================================================
 */
static struct work_struct g_work_struct[10];
static uint8_t g_work_struct_cnt = 0;

BOOLEAN RtmpOsTaskletInit(IN RTMP_ADAPTER *pAd, IN RTMP_NET_TASK_STRUCT
	*pTasklet, IN VOID(*pFunc)(unsigned long data), IN ULONG Data, IN
	LIST_HEADER *pTaskletList)
{

	#ifdef WORKQUEUE_BH
	#if 0
	if (RTMP_OS_Alloc_Rsc(pTaskletList, pTasklet, sizeof(struct work_struct)) ==
	FALSE)
	{
	...
	struct work_struct *work;
	//p_dbg_enter;
	if(g_work_struct_cnt < 10)
		work = &g_work_struct[g_work_struct_cnt++];
	else{
		work = (struct work_struct*)mem_malloc(sizeof(struct work_struct));
		if (!work)
		{
			p_err_mem;
			return FALSE;
	...
	pTasklet->pContent = (UCHAR*)work;
	work->data = (void*)Data;
	work->fun = (void*)pFunc;
	work->pTaskletList = pTaskletList;
	work->event = pAd->tasklet_thread_event;
	
	//	INIT_WORK((struct work_struct *)(pTasklet->pContent), pFunc);
	#else 

	if (RTMP_OS_Alloc_Rsc(pTaskletList, pTasklet, sizeof(OS_NET_TASK_STRUCT)) ==
	FALSE)
	{
		return FALSE; /* allocate fail */
	}
...
BOOLEAN RtmpOsTaskletKill(IN RTMP_NET_TASK_STRUCT *pTasklet)
{
	if (pTasklet->pContent != NULL)
	{
		...
		struct work_struct *work = (struct work_struct*)pTasklet->pContent;
		//p_dbg_enter;
...
VOID RtmpOsTaskletDataAssign(IN RTMP_NET_TASK_STRUCT *pTasklet, IN ULONG Data)
{
	#ifdef WORKQUEUE_BH
	struct work_struct *work;
	//	p_dbg_enter;
