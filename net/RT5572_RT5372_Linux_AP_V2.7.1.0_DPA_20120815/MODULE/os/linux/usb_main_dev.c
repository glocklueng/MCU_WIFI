	...
	IN VOID 				*pAd)
{
	...
	Config.ProductID = dev->descriptor->idProduct;
	Config.VendorID = dev->descriptor->idVendor;
...
/**************************************************************************/
/**************************************************************************/
int rtusb_probe(struct usb_device *dev);
...
{	
	...
	intf = dev->config->interface[0];
	
	rv = rt2870_probe(intf, dev, 0, &pAd);
	if (rv != 0)
	{
		p_err("%s err:%d\n", __FUNCTION__, rv);
		rv =  - 1;
...
/*NetDevInit============================================== */
	...
	usb_dev->net_dev = net_dev; //add by lck
...
/*	RTMP_DRIVER_CFG80211_INIT(pAd, usb_dev); */
	...
	*/
	CFG80211_Register(pAd, 0, net_dev);
}
#endif /* RT_CFG80211_SUPPORT */
...
err_out_free_radev:
	//RTMPFreeAdapter(pAd); //del by lck
