...
#ifdef MT7601
const uint32_t bulkout_ep_map[10] = {0xff, 0xff, 0xff, 0xff, 3, 4, 5, 6, 2, 7 };
const uint32_t bulkin_ep_map[8] = {0xff, 0xff, 0xff, 0xff, 0, 1};
#endif
...
USBHST_STATUS USBUploadFWComplete(URBCompleteStatus Status, purbb_t pURB, pregs *pt_regs)
{
 ...
 void DlListInit(struct _DL_LIST *List)
{
	List->Next = List;
	List->Prev = List;
}

 void DlListAdd(struct _DL_LIST *List, struct _DL_LIST *Item)
{
	Item->Next = List->Next;
	Item->Prev = List;
	List->Next->Prev = Item;
 ...
 void DlListAddTail(struct _DL_LIST *List, struct _DL_LIST *Item)
{
	DlListAdd(List->Prev, Item);
}

void DlListDel(struct _DL_LIST *Item)
{
...
int DlListEmpty(struct _DL_LIST *List)
{
	return List->Next == List;
}

unsigned int DlListLen(struct _DL_LIST *List)
{
...
loadfw_protect:
	...
	SentToMCUDone = (void *)RtmpInitCompletion();
...
again:		
			...
			if (!RtmpWaitForCompletionTimeout(SentToMCUDone, (1000)))
			{
	...
	RtmpRealseCompletion(SentToMCUDone);
...
INT USBKickOutCmd(PRTMP_ADAPTER pAd, UCHAR *Buf, UINT32 Len)
{
	...
	if (!RtmpWaitForCompletionTimeout(SentToMCUDone, (500)))
	{
		RTUSB_UNLINK_URB(pURB);
		Ret = NDIS_STATUS_FAILURE;
...
INT AsicSendCmdToAndes(PRTMP_ADAPTER pAd, struct CMD_UNIT *CmdUnit)
{
	...
	if (CmdUnit->u.ANDES.NeedRsp)
	{
		...
		if (CmdUnit->u.ANDES.NeedWait)
		{
			...
			CmdRspEvent->AckDone = (void*)RtmpInitCompletion();
		}
	...
	if (CmdUnit->u.ANDES.NeedWait) {
		...
		RTMP_IRQ_LOCK(&MCtrl->CmdRspEventListLock, IrqFlags);
		DlListDel(&CmdRspEvent->List);
		RtmpRealseCompletion(CmdRspEvent->AckDone);
		os_free_mem(NULL, CmdRspEvent);
		RTMP_IRQ_UNLOCK(&MCtrl->CmdRspEventListLock, IrqFlags);
	}
...
INT AndesRandomWritePair(PRTMP_ADAPTER pAd, RTMP_REG_PAIR *RegPair, UINT32 Num)
{
	...
	while (CurLen < VarLen)
	{
		...
		p_dbg("AsicSendCmdToAndes:%d, %d", SentLen, LastPacket);
		Ret = AsicSendCmdToAndes(pAd, &CmdUnit);
