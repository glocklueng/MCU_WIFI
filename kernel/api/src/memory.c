...
void mem_free(void *rmem)
{
	...
	SUB_MONITOR_ITEM_VALUE(mem_used, mem->next - (mem_size_t)(((unsigned char*)mem - ram)));
...
/*	if (p_ad)
	{
		if (once == 0)
		{
			p_ad->txpower_critcal_flag = 0x12345678;
			p_ad->channellist_critcal_flag = 0x12345678;
			p_ad->SharedKey_critcal_flag = 0x12345678;
			once = 1;
		}
		else
		{
			assert(p_ad->txpower_critcal_flag == 0x12345678);
			assert(p_ad->channellist_critcal_flag == 0x12345678);
			assert(p_ad->SharedKey_critcal_flag == 0x12345678);
		}
	}*/
